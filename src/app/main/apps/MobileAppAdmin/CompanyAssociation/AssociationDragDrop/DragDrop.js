...
const DragDrop = (props) => {
  ...
  // const [groupB, setGroupB] = useState(["Horse", "Cow", "Snake"]);
  ...
  useEffect(() => {
    // selectedCType
    setRetId(props.retId);
    setDistriId(props.distriId);

    if (props.selectedCType === "2") {
      //distributor list, selected retailer in parent
      let tempDistriComp = props.distriComp;
      // let temp = tempDistriComp.map(item => {
      let tempA = [];
      let tempB = [];
      tempDistriComp.map((item) => {
        if (item.associatedRetailer !== null) {
          if (props.retId === item.associatedRetailer.retailer_id) {
            tempB.push({
              id: item.id,
      ...
      setGroupA(tempA);
      setGroupB(tempB);
    } else if (props.selectedCType === "1") {
      let tempRetailerComp = props.retComp;
      let tempA = [];
      let tempB = [];
      tempRetailerComp.map((item) => {
        if (item.retailers.length > 0) {
          tempB.push({
            id: item.id,
            client_id: "",
            company: item.company_name,
            cityName: item.city_name?.name,
          });
        } else {
          tempA.push({
            id: item.id,
  ...
  const handleSubmit = (e) => {
    ...
    if (props.selectedCType === "1") {
      //here retailer list, selected distributor in parent
      // get id key from groupB array for associate id
      let associated_ids = groupB.map((item) => {
        return item.id;
      });
      Association(associated_ids);
    } else if (props.selectedCType === "2") {
      // here distributor list, selected retailer in parent
      //get client_id from groupB array for associate id
      let associated_ids = groupB.map((item) => {
  ...
  function Association(associated_ids) {
    let data = {
      ...
      ...(props.selectedCType === "1" && {
        distributor_id: distriId,
      }),
      ...(props.selectedCType === "2" && {
        retailer_id: retId,
      }),
    };
    ...
    axios
      .post(
        Config.getCommonUrl() + "api/retailerMaster/associate-client-retailer",
        data
      )
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "api/retailerMaster/associate-client-retailer",
          data,
        });
      });
  }
          ...
          <div className="p-5 pl-16 pr-16 text-left">
            ...
            {/* <div style={{ textAlign: "center", marginBottom: "2%" }}>
              <Button
                variant="contained"
                color="primary"
                className=" w-216 mx-auto mt-16 m-5 popup-cancel"
                onClick={handleClose}
              >
                Cancel
              </Button>
              ...
              <Button
                variant="contained"
                className="w-216 ml-16 mx-auto mt-16 m-5 popup-save"
