...
const TypeZeroComp = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.apiData?.id) {
      setUpdtRecord({
        ...
        unit : props.apiData.unit,
        adminnotes: props.apiData.admin_notes,
        errors: {
          karat: null,
          total_pieces: null,
          customer_remark: null,
  ...
  function validateRecord() {
    ...
    let tempDesignRecords = apiData.customer_designs.map((item) => {
      ...
      if(
        item.unit === "" || item.unit === undefined || item.unit === null
      ) {
        item.errors.unit = "Please Select Unit" ;
        pass = false;
      }
  ...
  function updateOrderApi() {
    ...
    axios
      .put(Config.getCommonUrl() + "api/order/change-order-design-info", {
        ...
        admin_notes: updtRecord.adminnotes
      })
    ...
    <>
      <Grid container spacing={3}  style={{marginTop:"20px"}}>
        ...
        <Grid item xs={4} style={{ padding: 5, display: "flex" }}>
          <label className={clsx( classes.tableRowPad,"text-15",classes.textOverFlow)}>
                ...
                <TableBody>
                  {apiData.customer_designs.map((data, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className={classes.tableRowPad}  style={{position:"relative"}}>
                        <select 
                        name="unit"
                        label="unit"
                           className={classes.normalSelect}
