...
const RemoveFromLot = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Remove_From_Lot_" + getDateAndTime(),
  ...
  useEffect(() => {
    if (selectedLotNumber) {
      getAddToLotListing(props.history.location.state?.data);
    }
  }, [selectedLotNumber]);
  ...
  function getSettingTypeId(batchNo, stockId, i) {
    Axios.get(
      Config.getCommonUrl() +
        `api/production/removelot/settingtype/list?batch_no=${batchNo}&stock_code_id=${stockId}`
    )
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: `api/production/removelot/settingtype/list?batch_no=${batchNo}&stock_code_id=${stockId}`,
        });
      });
  }
  function getGoldVariantStock(lotpurity) {
    const api = `api/production/removelot/metalStock?purity=${lotpurity}`;
  ...
  function getStockCode(batchNum, lId, wid) {
    const api = `api/production/stone/removefrom/lot?department_id=${department_id}&lot_id=${lId}&batch_no=${batchNum}&workstation_id=${wid}`;
    Axios.get(Config.getCommonUrl() + api)
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
  ...
  function clearDataTwo(fromSave) {
    // if (!fromSave) {
    //   setLotNumber("");
    //   setSelectedLotNumber("");
    // }
  ...
  function handleBatchOptionSelect(option, i) {
    ...
    getStockCode(
      option.label,
      apiDataArray.id,
      apiDataArray?.WorkStationDetails?.id
    );
  }
  ...
  function handleStockCodeSelect(option, i, data) {
    ...
    if (!isStockCodeAvailable) {
      ...
      dispatch(
        Actions.showMessage({
          message: "This stock code alredy selected",
        })
      );
    }
  }
  ...
  let handleChange = (e, i) => {
    ...
    if (name === "pcs") {
      ...
      const calculateWgt =
        parseFloat(addStockUpdatedData[i].stockCode?.data?.stone_weight) *
        parseFloat(pcsVal);
  ...
  function handleSubmitAddStock() {
    ...
    console.log(body);
    Axios.post(Config.getCommonUrl() + `api/production/remove`, body)
      .then(function (response) {
        if (response.data.success === true) {
          dispatch(
            Actions.showMessage({
  ...
  function validateAddStock() {
    if (addMetalData[0].lotNo === "" && addStockData[0].lotNo === "") {
      dispatch(
        Actions.showMessage({
          message: "Remove metal or remove any stock",
          variant: "error",
        })
      );
      return false;
