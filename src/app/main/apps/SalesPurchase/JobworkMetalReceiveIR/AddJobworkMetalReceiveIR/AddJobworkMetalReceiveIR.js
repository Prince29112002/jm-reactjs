...
import { AddjobworkMetalReceivelrPrint } from "../PrintComponent/AddjobworkMetalReceivelrPrint";
...
const AddJobworkMetalReceiveIR = React.memo((props) => {
...
function getDateAndTime() {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Jobwork Metal Receive Voucher_" + getDateAndTime(),
  ...
  useEffect(() => {
    ...
    if (idToBeView !== undefined) {
      ...
      getJobWorkMetalRecRecord(idToBeView.id);
    } else {
  ...
  function getJobWorkMetalRecRecord(id) {
    setLoading(true);
    let api = ""
    if(props.forDeletedVoucher){
      api = `api/jobworkmetalreceive/${id}?deleted_at=1`
    }else {
      api = `api/jobworkmetalreceive/${id}`
    }
    axios
        ...
        if (response.data.success === true) {
          ...
          if (response.data.hasOwnProperty("data")) {
            if (response.data.data !== null) {
              ...
              for (let item of finalData.JobWorkMetalReceiveOrder) {
                // console.log(item);
                tempArray.push({
                  stockCode: {
              ...
              setPrintObj({
                ...
                supplierGstUinNum:
                  finalData.ClientCompany.gst_number === null
                    ? "-"
                    : finalData.ClientCompany.gst_number,
                supPanNum: finalData.ClientCompany.pan_number,
                supState: finalData.ClientCompany.StateName
                  ? finalData.ClientCompany.StateName.name
                  : finalData.state_name.name,
                supCountry: finalData.ClientCompany.CountryName.name
                  ? finalData.ClientCompany.CountryName.name
                  : finalData.ClientCompany.CountryName.name,
                supStateCode:
                  finalData.ClientCompany.gst_number === null
                    ? "-"
                    : finalData.ClientCompany.gst_number.substring(0, 2),
                purcVoucherNum: finalData.voucher_no,
                partyInvNum: finalData.party_voucher_no,
                voucherDate: moment(
                  finalData.purchase_voucher_create_date
                ).format("DD-MM-YYYY"),
                placeOfSupply: finalData.ClientCompany.StateName.name
                  ? finalData.ClientCompany.StateName.name
                  : finalData.ClientCompany.StateName.name,
  ...
  function getVoucherNumber() {
    axios
      .get(Config.getCommonUrl() + "api/jobworkmetalreceive/get/voucher")
      .then(function (response) {
        console.log(response);
      ...
      .catch(function (error) {
        handleError(error, dispatch, {
          api: "api/jobworkmetalreceive/get/voucher",
        });
      });
  }
  ...
  function handleCompanyChange(value) {
    ...
    const index = clientCompanies.findIndex(
    ...
    setPrintObj({
      ...
      supStateCode: clientCompanies[index].gst_number
        ? clientCompanies[index].gst_number.substring(0, 2)
        : "",
      // purcVoucherNum: "",
      // partyInvNum: "",
      voucherDate: moment().format("DD-MM-YYYY"),
  ...
  function addMetalPurchase(resetFlag, toBePrint) {
    ...
    console.log(body);
    axios
      .post(Config.getCommonUrl() + "api/jobworkmetalreceive", body)
      .then(function (response) {
        console.log(response);
                ...
                <Grid item xs={7} sm={7} md={7} key="1" style={{ padding: 0 }}>
                  <FuseAnimate delay={300}>
                    <Typography className="pl-28 pt-16 text-18 font-700">
                      {isView
                        ? "View Jobwork Metal Receive"
                        : "Add Jobwork Metal Receive"}
                    </Typography>
                  </FuseAnimate>
                ...
                <div>
                  ...
                  >
                    ...
                    <div className="table_full_width ir-metal-purchase">
                      <div
                        className="mt-16 table-metal-purchase viewjobworkmetal_receive_dv viewjobworkmetal_receive_blg "
                        style={{
...
export default AddJobworkMetalReceiveIR;
