        ...
        <TableBody>
          {tagWiseData.map((element, index) => (
            <TableRow key={index}>
              ...
              <TableCell className={classes.tableRowPad}>
                {/* Job work Fine in Pure */}
                {element.jobWorkFineinPure ? parseFloat(element.jobWorkFineinPure).toFixed(3) : '-'}
              </TableCell>
              <TableCell className={classes.tableRowPad}>
                {/* Rate */}
                {element.rate ? isView ? Config.numWithComma(element.rate) : parseFloat(element.rate).toFixed(3) : '-'}
              </TableCell>
              <TableCell className={classes.tableRowPad}>
                {/* Valuation */}
                {element.valuation ? isView ? Config.numWithComma(element.valuation) :  parseFloat(element.valuation).toFixed(3) : '-'}
              </TableCell>
            </TableRow>
          ))}
