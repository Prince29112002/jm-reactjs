...
import { DocumentUploadRetailer } from "./DocumentUploadRetailer";
...
function ViewDocModelRetailer(props) {
  ...
  function callDeleteDocument() {
    ...
    Axios
      .delete(
        Config.getCommonUrl() + "retailerProduct/api/salespurchasedocs/" + selectedIdForDelete
      )
      ...
      .catch((error) => {
        setLoading(false)
        handleError(error, dispatch, { api: "retailerProduct/api/salespurchasedocs/" + selectedIdForDelete })
  ...
  useEffect(() => {
    ...
    if (docFile) {
      setLoading(true)
      DocumentUploadRetailer(docFile, props.purchase_flag, props.purchase_flag_id)
        .then((response) => {
          console.log(response)
        ...
        .catch((error) => {
          setLoading(false)
          handleError(error, dispatch, { api: "retailerProduct/api/salespurchasedocs/upload", body: docFile })
        })
    }
  }, [docFile])
  ...
  {!props.viewPopup &&
          <p className="ml-16 mt-16">Upload document</p>}
          {!props.viewPopup &&
            <TextField
...
export default ViewDocModelRetailer;
