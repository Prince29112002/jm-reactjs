...
const SalesB2C = (props) => {
  ...
  // useEffect(() => {
  //   let arr = roleOfUser
  //       ? roleOfUser["Sales Purchase"]["Sales B2C"]
  //         ? roleOfUser["Sales Purchase"]["Sales B2C"]
  //         : []
  //       : [];
  //   const arrData = [];
  ...
  function setFilters() {
    ...
    let url = "api/salesB2C?department_id=" + selectedDepartment.value.split("-")[1] + "&is_vendor_client=" + 4;
  ...
  function resetFilters() {
    ...
    getSalesDomestic("api/salesB2C?department_id=" + selectedDepartment.value.split("-")[1] + "&is_vendor_client=" + 4);
  }
  ...
  function viewHandler(id) {
    ...
    props.history.push("/dashboard/sales/salesb2c/addsalesb2c", {
      id: id
    });
  }
  ...
  function callDeleteRepairRecFromCust() {
    if(validateDelete()){
      ...
      axios
      .post(
        Config.getCommonUrl() + "api/salesB2C/" + selectedIdForDelete , body
      )
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
      ...
      .catch((error) => {
        handleError(error, dispatch, {api : "api/salesB2C/" + selectedIdForDelete , body})
      });
      handleClose()
    }
  }
      ...
      <FuseAnimate animation="transition.slideUpIn" delay={200}>
        ...
        <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1"style={{marginTop: "30px"}}>
            ...
            >
              ...
              {
                // authAccessArr.includes('Add Sales B2C') && 
                <Grid
                item
                xs={3}
                sm={4}
              ...
              >
                <Link
                  to="/dashboard/sales/salesb2c/addsalesb2c"
              ...
              <Paper className={clsx(classes.tabroot, "table-responsive new-add_stock_group_tbel")} id="salesdomestic-tabel-dv">
                <Table className={classes.table}>
                  ...
                  <TableBody>
                    {apiData.length === 0 ? (<>
                      <TableRow>
                        <TableCell className={classes.tableRowPad} colSpan={6} style={{textAlign:"center"}}>
                          No Data
                        </TableCell>
                      </TableRow>
                      </>) : apiData
                      ...
                      .map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell className={classes.tableRowPad}>
                            {
                              // authAccessArr.includes('View Sales B2C') && 
                               <IconButton
                              style={{ padding: "0" }}
                              onClick={(ev) => {
                                ev.preventDefault();
                            ...
                            >
                             ...
                             {
                              // authAccessArr.includes('Delete Sales B2C') && 
