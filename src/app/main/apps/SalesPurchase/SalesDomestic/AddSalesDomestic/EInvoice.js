...
function EInvoice(props) {
  ...
  useEffect(() => {
    // getheroID(props.InvoiceDate)
    console.log(props.InvoiceDate);
    // console.log(moment(props.InvoiceDate).format("DD-MM-YYYY"));
  ...
  function getPDF() {
    ...
    const body = { id: props.id.id };
    axios
      .post(Config.getCommonUrl() + "api/gstHero/invoice/genrate/pdf", body)
      .then(function (response) {
        console.log(response);
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "api/gstHero/invoice/genrate/pdf",
          body: body,
        });
      });
  }
