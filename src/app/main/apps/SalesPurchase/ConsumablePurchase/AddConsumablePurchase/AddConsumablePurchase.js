...
import { ConsumablePurchasePrint } from "../PrintComponent/ConsumablePurchasePrint";
...
const AddConsumablePurchase = ((props) => {
  ...
  function checkforPrint() {
    ...
    setPrintObj({
      stateId: stateId,
      supplierName: basicData?.firm_name || basicData?.company_name,
  ...
  useEffect(() => {
    getUnitData();
    setOppositeAccData(
      HelperFunc.getOppositeAccountDetails("ConsumablePurchase")
    );
  ...
  function getConsumePurchaseRecordForView(id) {
    ...
    if (props.forDeletedVoucher) {
      api = `api/consumablepurchase/${id}?deleted_at=1`;
    } else {
      api = `api/consumablepurchase/${id}`;
    }
    axios
        ...
        if (response.data.success === true) {
          ...
          if (response.data.hasOwnProperty("data")) {
            if (response.data.data !== null) {
              ...
              var state = "";
              if (finalData.ConsumablePurchaseOrder[0].igst) {
                state = 0;
                setStateId(0);
              } else {
                state = 12;
              ...
              for (let item of finalData.ConsumablePurchaseOrder) {
                tempArray.push({
                  stockCode: {
  ...
  function getVoucherNumber() {
    axios
      .get(Config.getCommonUrl() + "api/consumablepurchase/get/voucher")
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
  ...
  function addConsumablePurchase(resetFlag, toBePrint) {
    ...
    console.log(body);
    axios
      .post(Config.getCommonUrl() + "api/consumablepurchase", body)
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
          dispatch(Actions.showMessage({ message: response.data.message }));
  ...
  function getLedger(tcstds) {
    if (tcstds == 2) {
      var api = `api/ledgerMastar/tds/5`;
    } else if (tcstds == 1) {
      var api = `api/ledgerMastar/tcs/5`;
    }
    axios
      .get(Config.getCommonUrl() + api)
      .then(function (response) {
  ...
  function getTdsTcsVoucherNum(ledgerMasterId) {
    axios
      .get(
        Config.getCommonUrl() +
          `api/consumablepurchase/get/voucher/${ledgerMasterId}`
      )
      .then(function (response) {
        if (response.data.success === true) {
      ...
      .catch(function (error) {
        handleError(error, dispatch, {
          api: `api/consumablepurchase/get/voucher/${ledgerMasterId}`,
        });
      });
  }
  ...
  const handleStockGroupChange = (i, e) => {
    if (partyNameValidation()) {
      ...
      console.log(arrData.stock_name_code.unitName?.id);
      if (stateId === 12) {
  ...
  function getLastPrice(newFormValues, vendorId, stockCodeId, index) {
    axios
      .get(
        Config.getCommonUrl() +
          "api/consumablepurchase/lastprice/" +
          vendorId +
          "/" +
          stockCodeId
      )
      ...
      .catch(function (error) {
        handleError(error, dispatch, {
          api:
            "api/consumablepurchase/lastprice/" + vendorId + "/" + stockCodeId,
        });
      });
  }
...
export default AddConsumablePurchase;
