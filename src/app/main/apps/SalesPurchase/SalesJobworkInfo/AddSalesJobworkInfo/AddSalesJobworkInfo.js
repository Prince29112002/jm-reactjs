...
import { SalesJobworkInfoprint } from "../PrintComponent/SalesJobworkInfoprint";
...
const AddSalesJobworkInfo = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Sales Jobwork Info Voucher" + getDateAndTime(),
  ...
  useEffect(() => {
    console.log("useEffect isView ", isView)
  }, [isView]);
  ...
  function checkforPrint() {
    if (
      ...
      if (isView) {
        ...
        if (handleDateBlur() && selectedLoad.value === 0) {
  ...
  const loadTypeOptions = [
    { id: 0, name: "Load Packing Slip" },
    { id: 1, name: "Load Metal Variant" },
    { id: 2, name: "Load Findings Variant" },
    { id: 3, name: "Load Lot directly" },
  ]
  const shipToOtherPartyOption = [
    { id: 0, name: "No" },
    { id: 1, name: "Yes" },
  ]
  ...
  useEffect(() => {
    const timeout = setTimeout(() => {
      if (packingSearch) {
        if (fineRate === "") {
          dispatch(
            Actions.showMessage({ message: "Please Set Fine Rate First", variant: "error" })
          );
        } else {
          // if (selectedVoucherNm === "") {
          //   setSelectedVouErr("Please Enter Voucher Number");
          // } else {
  ...
  useEffect(() => {
    ...
    setOppositeAccData(HelperFunc.getOppositeAccountDetails("TempSalesJobwork"));

    console.log("idToBeView", idToBeView);
    if (idToBeView !== undefined) {
      setIsView(true);
      setNarrationFlag(true)
      getSalesJobWorkRecord(idToBeView.id);
    } else {
  ...
  useEffect(() => {
    if (idToBeView === undefined) {
      if (rateValue && selectedLoad.value === 0) {
        calculateFileRate()
      }
      else if (rateValue) {
        calculateFileRate()
        // changeTotal(formValues);
      }
    }
  ...
  const calculateFileRate = () => {
    ...
    tempLedgerAmount = parseFloat((parseFloat(tempTotalInvoiceAmt) * rateValue) / 100).toFixed(3); //calculating before gst, on total amount only
    ...
    // console.log("ledgerrrr", tempLedgerAmount)
    // console.log("finallllllll", tempfinalAmount)
  ...
  function getSalesJobWorkRecord(id) {
    setLoading(true);
    let api = ""
    if (props.forDeletedVoucher) {
      api = `api/tempSalesJobwork/${id}?deleted_at=1`
    } else {
      api = `api/tempSalesJobwork/${id}`
    }
    axios
        ...
        if (response.data.success === true) {
          ...
          if (response.data.hasOwnProperty("data")) {
            if (response.data.data !== null) {
              ...
              console.log(response.data.data.otherDetails.loadType);
              setSelectedLoad({ value: loadType, label: loadType === 0 ? "Load Packing Slip" : loadType === 1 ? "Load Metal Variant" : loadType === 2 ? "Load Findings Variant" : loadType === 3 ? "Load Lot Directly" : null })
              ...
              console.log(finalData.is_shipped);

              setShipping([
                { value: finalData.is_shipped, label: (finalData.is_shipped === 0 ? "No" : finalData.is_shipped === 1 ? "Yes" : null), key: finalData.is_shipped }
              ])
  ...
  function getVoucherNumber() {
    axios
      .get(Config.getCommonUrl() + "api/tempsalesJobwork/get/voucher")
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
  ...
  const prevIsValid = () => {
    ...
    const someEmpty = formValues
      .filter((element) =>
        selectedLoad.value === 3 ? element.lotno !== "" : element.category !== ""
      )
      .some((item) => {
        if (selectedLoad.value === 3) {
          if (
  ...
  function addUserInputApi(resetFlag, toBePrint) {
    ...
    if (Orders.length === 0) {
      dispatch(Actions.showMessage({ message: "Please Add Sales Entry", variant: "error" }));
      return;
    }
    setLoading(true);
    ...
    console.log(body);
    axios
      .post(Config.getCommonUrl() + "api/tempSalesJobwork", body)
      .then(function (response) {
        console.log(response);
        setLoading(false);
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, { api: "api/tempSalesJobwork", body: body })
      ...
      .then(function (response) {
        if (response.data.success === true) {
          ...
          if (response.data.data.length > 0) {
            ...
            dispatch(
              Actions.showMessage({
                message: "Please Insert Proper Packing Slip No", variant: "error"
              })
            );
          }
        } else {
  ...
  function createFromPackingSlip(resetFlag, toBePrint) {
    ...
    console.log(body);
    axios
      .post(Config.getCommonUrl() + "api/tempSalesJobwork/createFromPackingSlip", body)
      .then(function (response) {
        console.log(response);
        setLoading(false);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: "api/tempSalesJobwork/createFromPackingSlip", body: body })
      });
  }
        ...
        <div className="flex flex-col md:flex-row container">
          <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1 pt-20">            {
              ...
              <Grid item xs={7} sm={7} md={7} key="1" style={{ padding: 0 }}>
                <FuseAnimate delay={300}>
                  <Typography className="pl-28 pt-16 text-18 font-700">
                    ...
                    {isView ? "View Sales Info Invoice (Jobwork)" : "Add Sales Info Invoice (Jobwork)"}
                  </Typography>
                </FuseAnimate>
              ...
              >
                <Grid container spacing={3}>
                  ...
                  >
                    ...
                    <option value="1">Load Metal Variant</option>
                    <option value="2">Load Findings Variant</option>
                  ...
                  {allowedBackDate && (
                    <Grid
                      ...
                      style={{ padding: 6 }}
                    >
                      <p style={{ paddingBottom: "3px" }}>Voucher Date</p>
                  ...
                  <Grid item lg={2} md={4} sm={4} xs={12} style={{ padding: 6 }}>
                    ...
                    <Select
                      ...
                      classes={classes}
                      styles={selectStyles}
                      options={shipToOtherPartyOption.map((suggestion) => ({
                        value: suggestion.id,
                        label: suggestion.name,
                        key: suggestion.id
                      }))}
                      ...
                      placeholder="Ship To Other Party"
                      isDisabled={isView}
                      required
                    />
                    {/* <select
                ...
                {(selectedLoad.value === 1 ||
                  selectedLoad.value === 2 ||
                  selectedLoad.value === 3) && (
                        ...
                        >
                          ...
                          {formValues.map((element, index) => (
                            ...
                            >
                              ...
                              {selectedLoad.value === 1 && (
                                <>
                                  <Select
                                    ...
                                    className={clsx(classes.selectBox, "inputdropdown_select_dv")}
                                    classes={classes}
                                    styles={selectStyles}
                              ...
                              {(selectedLoad.value === 1 || selectedLoad.value === 2) && (
                                <TextField
                              ...
                              <TextField
                                ...
                                variant="outlined"
                                fullWidth
                                disabled={isView || selectedLoad.value === 3}
                              />
                              ...
                              {(element.grossWeight !== "" && element.netWeight !== "" && (parseFloat(element.grossWeight).toFixed(3) !== parseFloat(element.netWeight).toFixed(3)) && selectedLoad.value !== 3) && (
                                <IconButton
                              ...
                              <TextField
                                ...
                                className=""
                                value={isView ? Config.numWithComma(element?.wastageFineamount) : element.wastageFineamount || ""}
                                error={
                                  element.errors !== undefined
                              ...
                              <TextField
                                ...
                                className=""
                                value={isView ? Config.numWithComma(element?.billingLabourRateperGram) : isNaN(element.billingLabourRateperGram) ? " " : element.billingLabourRateperGram || ""}
                                error={
                                  element.errors !== undefined
                ...
                {!props.viewPopup && <div>
                  ...
                  <div style={{ display: "none" }}>
                    <SalesJobworkInfoprint ref={componentRef} printObj={printObj} 
                    isView={isView} getDateAndTime={getDateAndTime()} />
                  </div>
            ...
            <ViewDocModal documentList={documentList} handleClose={handleDocModalClose} open={docModal} updateDocument={updateDocumentArray}
              purchase_flag_id={idToBeView?.id} purchase_flag="32" concateDocument={concateDocument} viewPopup={props.viewPopup}
            />
...
export default AddSalesJobworkInfo;