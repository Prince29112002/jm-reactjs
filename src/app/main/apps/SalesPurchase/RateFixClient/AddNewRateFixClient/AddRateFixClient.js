...
const AddRateFixClient = (props) => {
  ...
  function getClientFirmData(clientId) {
    axios
      ...
      .catch(function (error) {
        handleError(error, dispatch,{api:"api/ratefix/company/" + clientId})
  ...
  function getVoucherNumber() {
    axios
      .get(Config.getCommonUrl() + "api/ratefix/get/voucher?party_type=1")//client
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
      ...
      .catch(function (error) {
        handleError(error, dispatch, {api : "api/ratefix/get/voucher?party_type=1"})
  ...
  function handleFirmChange(value) {
    ...
    // if (clientSelected !== "") {
    //   getRateFixCompnyWise(clientSelected.value, value.value);
    // }

    // value.value is id field so we have to get index of this id and set gst number from index array
    // getRateFixDataForClient(value.value);
  }
  ...
  function oldvoucherNumValidation() {
    if (refVoucher === "") {
      dispatch(
        Actions.showMessage({
          message: "Select Voucher from list",
          variant: "error",
        })
      );
      return false;
  ...
  function callOldRateFixApi() {
    const body = {
      party_type: 1,
      old_new: 0,
      party_id: clientSelected.value,
      client_company_id: firmSelected.value,
      weight: fine,
      rate: tempRate,
  ...
  function validatePartynameFirmName() {

    if (clientSelected === "" || firmSelected === "") {
      if (clientSelected === "") {
  ...
  function callTempRateFixApi() {
    setLoading(true);
    axios.get(Config.getCommonUrl() + `api/ratefix/temp/rate/${clientSelected.value}/${firmSelected.value}`)
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
      ...
      .catch(function (error) {
        // console.log(error);
        setLoading(false);
        handleError(error, dispatch, {api : `api/ratefix/temp/rate/${clientSelected.value}/${firmSelected.value}`})
            ...
            <div className="main-div-alll ">
              ...
              >
                ...
                <div>
                  ...
                  >
                    {/* <div className="my-16" style={{ width: "20%" }}> */}
                    <Grid
                    ...
                    >
                      ...
                      <Grid item lg={4} md={4} sm={4} xs={12} style={{ marginTop: -10 }}>
                      ...
                      <>
                        ...
                        >
                          ...
                          <Grid item lg={4} md={4} sm={4} xs={12} style={{ padding: 6 }}>
                            ...
                            {/* <label className="ml-2">Gram</label> */}
                          </Grid>
                          ...
                          <Grid item lg={4} md={4} sm={4} xs={12} style={{ padding: 6 }}>
                            ...
                            />
                            {/* <label className="ml-2"> / 10 Gram</label> */}
                          </Grid>

                          {/* <Grid item lg={1} md={1} sm={3} xs={12} style={{ padding: 6,marginTop:25 }}>
                          <label style={{ width: "30%" }} className="ml-2"> / 10 Gram</label>
                          </Grid> */}
                        </Grid>
                      ...
                      <div>
                        <div>
                          <Paper
                            className={clsx(
                              classes.tabroot,
                              "table-responsive addratefix_client_dv"
                            )}
                          >
                              ...
                              <TableBody>
                                {tempRateList.map((row, i) => (
                                    ...
                                    <TableCell className={classes.tableRowPad}>
                                      <Checkbox type="checkbox" value={JSON.stringify(row)} onChange={handleSelectRateList} checked={selectedRateList === row.id ? true : false} />
                                    </TableCell>
                                    <TableCell className={classes.tableRowPad}>
                                      {row.voucher_no}
                                    </TableCell>
...
export default AddRateFixClient;
