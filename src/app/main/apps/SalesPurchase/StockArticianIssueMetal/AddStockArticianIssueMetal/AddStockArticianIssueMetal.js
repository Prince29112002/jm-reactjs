...
const AddStockArticianIssueMetal = (props) => {
  ...
  const loadTypeOptions = [
    {id: 0, name: "Load Metal Variant"},
    {id: 1, name: "Load Lot Directly"},
  ]
  ...
  function getArticianIssueRecord(id) {
    ...
    if(props.forDeletedVoucher){
      api = `api/stockJVjobworkarticianissue/jv/${id}?deleted_at=1`
    }else {
      api = `api/stockJVjobworkarticianissue/jv/${id}`
    }
    axios
        ...
        if (response.data.success === true) {
          ...
          if (response.data.hasOwnProperty("data")) {
            if (response.data.data !== null) {
              ...
              setSelectedLoad({value:loadType, label: loadType===0? "Load Metal Variant":loadType===1?"Load Lot Directly": null})
              ...
              let tempStateId = finalData.StockJVJobWorker.state_name.id;
              ...
              if (loadType === 0) {
                ...
                for (let item of finalData.StockJVJobWorkArticianIssueOrder) {
                  // console.log(item);
                  let tCgstPer = tempStateId === 12 ? parseFloat(parseFloat(item.StockJVStockNameCode.stock_name_code.hsn_master.gst) / 2).toFixed(3) : ""
                  let tSgstper = tempStateId === 12 ? parseFloat(parseFloat(item.StockJVStockNameCode.stock_name_code.hsn_master.gst) / 2).toFixed(3) : ""
                  let tIgstPer = tempStateId !== 12 ? parseFloat(item.StockJVStockNameCode.stock_name_code.hsn_master.gst).toFixed(3) : "";
                  ...
                  tempArray.push({
                    ...
                    HSNnum: item.StockJVStockNameCode.stock_name_code.hsn_master.hsn_number,
                    // selectedHsn: item.StockJVStockNameCode.stock_name_code.hsn_master.hsn_number,
                    pcs: item.pcs,
                ...
                for (let item of finalData.StockJVJobWorkArticianIssueOrder) {
                  // console.log(item);
                  let tCgstPer = tempStateId === 12 ? parseFloat(parseFloat(item.StockJVCategory.hsn_master.gst) / 2).toFixed(3) : ""
                  let tSgstper = tempStateId === 12 ? parseFloat(parseFloat(item.StockJVCategory.hsn_master.gst) / 2).toFixed(3) : ""
                  let tIgstPer = tempStateId !== 12 ? parseFloat(item.Category.StockJVCategory.gst).toFixed(3) : "";
                  ...
                  tempArray.push({
                    ...
                    lotno: {
                      value: item.StockJVLot.id,
                      label: item.StockJVLot.number
                    },
                    categoryName: item.StockJVCategory.category_name,
                    grossWeight: parseFloat(item.gross_weight).toFixed(3),
                    netWeight: parseFloat(item.net_weight).toFixed(3),
                    pcs: item.pcs,
                    HSNnum: item.StockJVCategory.hsn_master.hsn_number,
                    purity: item.purity.toString(),
                    fineGold: parseFloat(item.finegold).toFixed(3),
                    rate: parseFloat(item.rate_of_fine_gold).toFixed(3),
                    amount: parseFloat(item.amount).toFixed(3),
              ...
              setPrintObj({
                ...
                supplierGstUinNum: finalData.StockJVJobWorker.gst_number === null ? "" : finalData.StockJVJobWorker.gst_number,
                ...
                supStateCode: finalData.StockJVJobWorker.gst_number === null ? "-" : finalData.StockJVJobWorker.gst_number.substring(0, 2),
  ...
  function getStockCodeAll() {
    axios
      .get(Config.getCommonUrl() + "api/stockJVjobWorkArticianIssue/goldStockCodes?department_id=" +
        window.localStorage.getItem("SelectedDepartment")
        // "api/stockname"
      )
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
          const filteredData = response.data.data.filter(item => parseFloat(item.weight) !== 0);
          setStockCodeData(filteredData);
        } else {
          dispatch(Actions.showMessage({ message: response.data.message }));
        }
      })
      .catch((error) => {
        handleError(error, dispatch, {
          api: "api/stockJVjobWorkArticianIssue/goldStockCodes?department_id=" +
            window.localStorage.getItem("SelectedDepartment")
        })
      });
  }
  ...
  function getVoucherNumber() {
    axios
      .get(Config.getCommonUrl() + "api/stockJVjobWorkArticianIssue/jv/get/voucher")
      .then(function (response) {
        console.log(response);
      ...
      .catch(function (error) {
        handleError(error, dispatch, { api: "api/stockJVjobWorkArticianIssue/jv/get/voucher" })
      });
  }
  ...
  function addArticianIssueApi(resetFlag, toBePrint) {
    ...
    const body = {
      ...
      is_lot: selectedLoad?.value === 1 ? 1 : 0,
    ...
    console.log(body)
    axios
      .post(Config.getCommonUrl() + "api/stockJVjobworkarticianissue/jv", body)
      .then(function (response) {
        console.log(response);
      ...
      .catch((error) => {
        // console.log(error);
        setLoading(false);
        console.log("errrrrr000000000");
        handleError(error, dispatch, { api: "api/stockJVjobworkarticianissue/jv", body: body })
      })
  }
  ...
  const prevContactIsValid = () => {
    ...
    const someEmpty = formValues
      .filter((element) =>
        selectedLoad.value === 1 ? element.lotno !== "" : element.stockCode !== ""
      )
      .some((item) => {
        if (selectedLoad.value === 1) {//lot
          return (
            item.categoryName === "" ||
            item.grossWeight === "" ||
            item.grossWeight ==0 ||
  ...
  let handlerBlur = (i, e) => {
    ...
    if (nm === "grossWeight" && selectedLoad.value === 0) {
  ...
  let handleChange = (i, e) => {
    ...
    if (newFormValues[i].rate !== "" && newFormValues[i].fineGold !== "") {
      ...
      //     10
      // ).toFixed(3);
      if (newFormValues[i].stock_group === 1 || selectedLoad.value === 1) {
        newFormValues[i].amount = parseFloat(
          (parseFloat(newFormValues[i].rate) *
            parseFloat(newFormValues[i].fineGold)) /
          10
      ...
      <FuseAnimate animation="transition.slideUpIn" delay={200}>
        <div className="flex flex-col md:flex-row container">
          <div className="flex flex-1 flex-col min-w-0" style={{ marginTop: "30px" }}>
              ...
              <Grid item xs={7} sm={7} md={7} key="1">
                <FuseAnimate delay={300}>
                  <Typography className="text-18 font-700">
                    {isView ? "View Artician Issue Metal (Stock Journal Voucher)" : "Add Artician Issue Metal (Stock Journal Voucher)"}
                  </Typography>
                </FuseAnimate>
              </Grid>
              <Grid
            ...
            <div
              className="articianissue-resturn-form"
            >
              <div>
                <form
                  name="registerForm"
                ...
                >
                  <Grid container spacing={3} className="p-8" alignItems="flex-end">
                    <Grid
                      item
                      lg={2}
                      md={4}
                    ...
                    >
                      ...
                      <Select
                        ...
                        classes={classes}
                        styles={selectStyles}
                        options={loadTypeOptions.map((suggestion) => ({
                          value: suggestion.id,
...
{/* {console.log(moment(voucherDate).format("YYYY-MM-DD"))} */}
                    <Grid
                      item
                      lg={2}
                      md={4}
                  ...
                  <div className="table_full_width add-articianissue-tbl">
                    <div
                      className="mt-16 inner-add-articianissue-tbl view_artician_issue_dv "
                      style={{
                        paddingBottom: 5,
                      }}
                    >
                      ...
                      >
                        ...
                        {selectedLoad.value === 1 && (
                          <div className={classes.tableheader}>Lot Number</div>
                        )}
                        {selectedLoad.value === 0 && (
                      ...
                      {formValues.map((element, index) => (
                        <div key={index} className="castum-row-dv">
                            ...
                            <>
                              <Select
                                ...
                                options={stockCodeData
                                  .filter(array => formValues.every(item => (!(item.stockCode?.value === array.stock_name_code.id && item.stockCode.label === array.stock_name_code.stock_code))))
                                  .map((suggestion) => ({
                          ...
                          <TextField
                            ...
                            onChange={(e) => handleChange(index, e)}
                            disabled={isView || selectedLoad.value === 1}//|| element.stock_group !== 5
                          ...
                          <TextField
                            ...
                            inputRef={(el) => (wtInputRef.current[index] = el)}
                            variant="outlined"
                            fullWidth
                            disabled={isView|| selectedLoad.value === 1}
                          />
                          <TextField
                           name="netWeight"
                          ...
                          <TextField
                            ...
                            disabled
                          />
                          {console.log("element.fineGold", element.fineGold)}
                          <TextField
                            // label="Fine Gold"
                            name="fineGold"
                            className=""
                      ...
                      <div
                        ...
                        style={{ fontWeight: "700", height: "30px", background: "#ebeefb" }}
                      >
                        {!isView &&
                        ...
                        >
                          {HelperFunc.getTotalOfFieldNoDecimal(formValues, "pcs")}
                          {/* pcs */}
                        </div>
                        <div className={clsx(classes.tableheader, "castum-width-table")}>
                          {totalGrossWeight}
                ...
                <div style={{ display: "none" }}>
                    <ArticianIssuePrintComp ref={componentRef} printObj={printObj} />
                  </div>
                </div>}
                {isView &&
                  <Button
                    variant="contained"
                    className={classes.button} onClick={() => setDocModal(true)} style={{marginTop: 16, marginLeft: 0}}>
                    View Documents
                  </Button>
                }

                <ViewDocModal documentList={documentList} handleClose={handleDocModalClose} open={docModal} updateDocument={updateDocumentArray}
                  purchase_flag_id={idToBeView?.id} purchase_flag="39" concateDocument={concateDocument} viewPopup={props.viewPopup} />
...
export default AddStockArticianIssueMetal;
