...
import { SalesDomesticInfoPrint } from "../PrintComponent/SalesDomesticInfoPrint";
...
const AddSalesDomesticInfo = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Sales Domestic Info Voucher " + getDateAndTime(),
  ...
  useEffect(() => {
    ...
    setOppositeAccData(HelperFunc.getOppositeAccountDetails("TempSalesDomestic"));
  ...
  function calculateWhenLedgerChange(){
    ...
    if (is_tds_tcs == 1) {
      ...
      tempLedgerAmount = parseFloat(
        (totalInvoiceAmount * rateValue) / 100
      ).toFixed(3); //with gst on total invoice amount
      console.log(tempLedgerAmount);

      tempfinalAmount = parseFloat(parseFloat(totalInvoiceAmount) + parseFloat(tempLedgerAmount)).toFixed(3);
  ...
  function getSalesDomesticRecord(id) {
    ...
    if(props.forDeletedVoucher){
      api = `api/tempSalesDomestic/${id}?deleted_at=1`
    }else {
      api = `api/tempSalesDomestic/${id}`
    }
    axios
  ...
  function getVoucherNumber() {
    axios
      .get(Config.getCommonUrl() + "api/tempsalesDomestic/get/voucher")
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
  ...
  function addUserInputApi(resetFlag, toBePrint) {
    ...
    console.log(body);
    axios
      .post(Config.getCommonUrl() + "api/tempSalesDomestic", body)
      .then(function (response) {
        console.log(response);
        setLoading(false);
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, { api: "api/tempSalesDomestic", body: body })
  ...
  function createFromPackingSlip(resetFlag, toBePrint) {
    ...
    console.log(body)
    axios
      .post(Config.getCommonUrl() + "api/tempSalesDomestic/createFromPackingSlip", body)
      .then(function (response) {
        console.log(response);
        setLoading(false);
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, { api: "api/tempSalesDomestic/createFromPackingSlip", body: body })
        ...
        <div className="flex flex-col md:flex-row container">
          <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1 pt-20">
              ...
              <Grid item xs={7} sm={7} md={7} key="1" style={{ padding: 0 }}>
                <FuseAnimate delay={300}>
                  <Typography className="pl-28 pt-16 text-18 font-700">
                    ...
                    {isView ? "View Sales Info Invoice (Domestic)" : "Add Sales Info Invoice (Domestic)"}
                  </Typography>
                </FuseAnimate>
            ...
            <div className="main-div-alll">
              ...
              >
                <Grid container spacing={3}>
                  ...
                  <Grid item lg={2} md={4} sm={4} xs={12} style={{ padding: 5 }}> <p style={{ paddingBottom: "3px" }}>
            ...
            <ViewDocModal documentList={documentList} handleClose={handleDocModalClose} open={docModal} updateDocument={updateDocumentArray}
              purchase_flag_id={idToBeView?.id} purchase_flag="31" concateDocument={concateDocument} viewPopup={props.viewPopup} />
...
export default AddSalesDomesticInfo;
