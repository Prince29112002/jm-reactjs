...
const SalesDomesticInfo = (props) => {
  ...
  //  useEffect(() => {
  //    let arr = roleOfUser
  //        ? roleOfUser["Sales Purchase"]["Sales Domestic Info"]
  //          ? roleOfUser["Sales Purchase"]["Sales Domestic Info"]
  //          : []
  //        : [];
  //    const arrData = [];
  ...
  //  }, []);
   ...
   function resetFilters() {
     ...
     getSalesDomestic("api/tempSalesDomestic?");
   }
   ...
   function viewHandler(id) {
     ...
     props.history.push("/dashboard/sales/domesticsaleinfo/adddomesticsaleinfo", {
       id: id
     });
   }
   ...
   function callDeleteRepairRecFromCust() {
     if(validateDelete()){
       ...
       axios
       .post(
         Config.getCommonUrl() + "api/tempSalesDomestic/" + selectedIdForDelete , body
       )
       .then(function (response) {
         console.log(response);
         if (response.data.success === true) {
       ...
       .catch((error) => {
         handleError(error, dispatch, {api : "api/tempSalesDomestic/" + selectedIdForDelete , body})
       });
       handleClose()
     }
   }
      ...
      <FuseAnimate animation="transition.slideUpIn" delay={200}>
        ...
        <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1" style={{marginTop: "30px"}}>
            ...
            >
              ...
              {
                // authAccessArr.includes('Add Sales Domestic Info') &&
                 <Grid
                item
                xs={3}
                sm={4}
              ...
              >
                <Link
                  to="/dashboard/sales/domesticsaleinfo/adddomesticsaleinfo"
                      ...
                      .map((row) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell
                            ...
                            className={classes.tableRowPad}
                          >
                          {row.is_vendor_client === 1 ? row.Vendor.name : row.Client.name}
                          </TableCell>
                          ...
                          <TableCell
                            ...
                            className={classes.tableRowPad}
                          >
                          {row.is_vendor_client === 1 ? row.Vendor.firm_name : row.ClientCompany.company_name} 
                          </TableCell>
                          <TableCell
                            align="left"
                            className={classes.tableRowPad}
                          ...
                          <TableCell className={classes.tableRowPad}>
                            {
                              // authAccessArr.includes('View Sales Domestic Info') && 
                               <IconButton
                              style={{ padding: "0" }}
                              onClick={(ev) => {
                                ev.preventDefault();
                            ...
                            >
                             ...
                             {
                              // authAccessArr.includes('Delete Sales Domestic Info') && 
...
export default SalesDomesticInfo;
