...
const ChangeStatus = (props) => {
  ...
  function Statuslist() {
    axios
      .get(Config.getCommonUrl() + "api/ProductionOrder/statusDropDown")
  ...
  function orderlist() {
    axios
      .get(
        Config.getCommonUrl() +
          `api/ProductionOrder/get/history/${props.orderId}`
      )
      .then(function (response) {
        if (response.data.success === true) {
          console.log(response);
          setOrderHis(response.data.data.rows);
          setoldorder(response.data.data.rows[0].order_status_text);
          setoldremark(response.data.data.rows[0].remark);
        }
      })
      .catch((error) => {
        handleError(error, dispatch, {
          api: `api/ProductionOrder/get/history/${props.orderId}`,
        });
      });
  }
  function callChangeStatusApi() {
    axios
      .put(Config.getCommonUrl() + "api/ProductionOrder/change-status", {
        order_status: statusSelected.value,
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "api/ProductionOrder/change-status",
          body: {
            order_status: statusSelected.value,
            order_id: props.orderId,
            remark: userName,
