...
const TagSystemRetailer = (props) => {
  ...
  : null;
    ...
    useEffect(() => {
      let arr = roleOfUser
          ? roleOfUser["Master-Retailer"]["Tag System-Retailer"]
            ? roleOfUser["Master-Retailer"]["Tag System-Retailer"]
            : []
          : [];
      const arrData = [];
  ...
  function SystemNameValidation() {
    if (systemName === "") {
      ...
      setSystemNameErr("Enter system name");
  ...
  function callDeleteTagSystemApi() {
    axios
      .delete(Config.getCommonUrl() + "retailerProduct/api/tagsystem/" + selectedIdForDelete)
      .then(function (response) {
        console.log(response);
        setOpen(false);
        if (response.data.success === true) {
      ...
      .catch((error) => {
        setApiData([])
        setOpen(false);
        handleError(error, dispatch, {api : "retailerProduct/api/tagsystem/" + selectedIdForDelete})
      });
  }
  ...
  function callAddTagSystemDetailsApi() {
    //add item type
    axios
      .post(Config.getCommonUrl() + "retailerProduct/api/tagsystem", {
        name: systemName,
      ...
      .catch((error) => {
        handleError(error, dispatch, {api : "retailerProduct/api/tagsystem", body: {
          name: systemName,
          original_name: systemMac,
        }})
      });
  ...
  function callEditTagSystemDetailsApi() {
    axios
      .put(Config.getCommonUrl() + "retailerProduct/api/tagsystem/" + selectedIdForEdit, {
        name: systemName,
        original_name: systemMac,
      })
      .then(function (response) {
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {api : "retailerProduct/api/tagsystem/" + selectedIdForEdit, body : {
          name: systemName,
          original_name: systemMac,
        }})
      });
      ...
      <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1">
        ...
        <div className="flex flex-1 flex-col min-w-0">
          ...
          >
            ...
            {
              authAccessArr.includes('Add /Edit Tag System-Retailer') && <Grid
              item
              xs={6}
              sm={8}
              md={8}
              ...
              <TableBody>
                ...
                .filter(
                  ...
                  <TableRow key={row.id}>
                    ...
                    <TableCell className={classes.tableRowPad}>
                      {
                        authAccessArr.includes('Add /Edit Tag System-Retailer') &&   <IconButton
                        style={{ padding: "0"}}
                        onClick={(ev) => {
                          ev.preventDefault();
                          ev.stopPropagation();
                    ...
                    {
                      authAccessArr.includes('View Tag System-Retailer') && <IconButton
                      style={{ padding: "0" }}
                      onClick={(ev) => {
                        ev.preventDefault();
                        ev.stopPropagation();
                    ...
                    {
                      authAccessArr.includes('Delete Tag System-Retailer') &&  <IconButton
                      style={{ padding: "0" }}
                      onClick={(ev) => {
                        ev.preventDefault();
                        ev.stopPropagation();
          ...
          >
            <div style={modalStyle} className={clsx(classes.paper,"rounded-8")}>
              ...
              >
              {isEdit ? "Edit System Details" : isView ? "View System Details" : "Add New System Details"}
                <IconButton
              ...
              <p className="popup-labl">System Name</p>
                <TextField
...
export default TagSystemRetailer;
