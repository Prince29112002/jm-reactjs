...
const StockNameView = (props) => {
  ...
  function getStockName(text) {
    ...
    axios
      .get(Config.getCommonUrl() + `retailerProduct/api/stockname/search/${search}`)
      ...
      .catch(function (error) {
        setLoading(false);
        handleError(error, dispatch, { api: `retailerProduct/api/stockname/search/${search}` });
      });
  }
  useEffect(()=>{
    if(props.permitArr){
  ...
  function callDeleteStockNameApi() {
    axios
      .delete(Config.getCommonUrl() + "retailerProduct/api/stockname/" + selectedIdForDelete)
      .then(function (response) {
        console.log(response);
        setOpen(false);
        if (response.data.success === true) {
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "retailerProduct/api/stockname/" + selectedIdForDelete,
        });
      });
  }
                ...
                {apiSearchData.length > 0 ? (
                  apiSearchData.map((row, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className={classes.tableRowPad}>
{                    authAccessArr.includes('Edit Stock Type-Retailer') && 
                        <IconButton
                          style={{ padding: "0" }}
                          onClick={(ev) => {
                            ev.preventDefault();
                        ...
                        {                    authAccessArr.includes('Delete Stock Type-Retailer') && 
