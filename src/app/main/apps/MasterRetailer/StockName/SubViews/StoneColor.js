...
const StoneColor = (props) => {
  ...
  const [editStoneColor, setEditStoneColor] = useState("");
  const [edtStoneColorErrTxt, setEdtStoneColorErrTxt] = useState("");
  const [authAccessArr, setAuthAccessArr] = useState([]);

  const handleSubmit = (evt) => {
    evt.preventDefault();
    if (stoneColorValidation()) {
      callAddStoneSizeApi();
    }
  };

  function stoneColorValidation() {
    var Regex = /^[a-zA-Z\s]*$/;
    if (!StoneColor || Regex.test(StoneColor) === false) {
      setStoneColorErrTxt("Please Enter Valid Stone Color");
  ...
  function callAddStoneSizeApi() {
    axios
      .post(Config.getCommonUrl() + "retailerProduct/api/stonecolor", {
        name: StoneColor,
      })
      .then(function (response) {
        console.log(response);
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "retailerProduct/api/stonecolor",
          body: {
            name: StoneColor,
          },
        });
  ...
  function handleInputChange(event) {
    ...
    if (name === "edtstoneColor") {
      setEditStoneColor(value);
      setEdtStoneColorErrTxt("");
    }
  }

  function editHandler(id, color) {
    setSelectedIdForEdit(id);
    setEditStoneColor(color);
  ...
  const checkforUpdate = (evt) => {
    ...
    if (edtStoneColorValidation()) {
      callEditStoneColorApi();
    }
  };

  function edtStoneColorValidation() {
    var Regex = /^[a-zA-Z\s]*$/;
    if (!editStoneColor || Regex.test(editStoneColor) === false) {
      setEdtStoneColorErrTxt("Please Enter Valid Stone Color");
  ...
  function callEditStoneColorApi() {
    axios
      .put(Config.getCommonUrl() + "retailerProduct/api/stonecolor/" + selectedIdForEdit, {
        name: editStoneColor,
      })
      .then(function (response) {
        console.log(response);
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "retailerProduct/api/stonecolor/" + selectedIdForEdit,
          body: {
            name: editStoneColor,
          },
        });
  ...
  function callDeleteStoneSizeApi() {
    axios
      .delete(Config.getCommonUrl() + "retailerProduct/api/stonecolor/" + selectedIdForDelete)
      .then(function (response) {
        console.log(response);
        setOpen(false);
        if (response.data.success === true) {
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "retailerProduct/api/stonecolor/" + selectedIdForDelete,
        });
      });
  }
    ...
    <div className={clsx(classes.root, props.className)}>
      ...
      >
        <div className="flex flex-1 flex-col min-w-0">
          ...
          >
            <div>
            {authAccessArr.includes('Add Stone Color-Retailer') && 
              <form className={classes.form} onSubmit={handleSubmit}>
                <label> Add Stone Color </label>
                <br></br>
                <input
                  // {classes.item}
          ...
          <Paper
            className={clsx(
              classes.tabroot,
              "stonecolor_tabel_dv srockname-tabel-bdr"
            )}
          >
                ...
                {apiData
                  ...
                  .map((row) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell className={classes.tableRowPad}>
                       {authAccessArr.includes('Edit Stone Color-Retailer') &&
                        ...
                        {authAccessArr.includes('Delete Stone Color-Retailer') &&
            ...
            >
              ...
              <div className="pl-32 pr-32 pb-10 pt-10">
                ...
                <TextField
                  ...
                  placeholder="Stone Color"
                  name="edtstoneColor"
                  value={editStoneColor}
                  error={edtStoneColorErrTxt.length > 0 ? true : false}
                  helperText={edtStoneColorErrTxt}
