...
const Unit = (props) => {
  ...
  function CallAddUnitApi() {
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + "retailerProduct/api/unitofmeasurement/", {
        unit_name: unitName,
        shortname: shortName,
      })
      .then(function (response) {
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: "retailerProduct/api/unitofmeasurement/",
          body: {
            // name: processNm,
            unit_name: unitName,
            shortname: shortName,
  ...
  function CallEditUnitApi() {
    ...
    axios
      .put(
        Config.getCommonUrl() + "retailerProduct/api/unitofmeasurement/" + selectedIdForEdit,
        {
          unit_name: unitName,
          shortname: shortName,
        }
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: "retailerProduct/api/unitofmeasurement/" + selectedIdForEdit,
          body: {
            unit_name: unitName,
            shortname: shortName,
          },
  ...
  function callDeleteProcessApi() {
    axios
      .delete(
        Config.getCommonUrl() + "retailerProduct/api/unitofmeasurement/" + selectedIdForDelete
      )
      .then(function (response) {
        console.log(response);
        setOpen(false);
      ...
      .catch((error) => {
        setOpen(false);
        handleError(error, dispatch, {
          api: "retailerProduct/api/unitofmeasurement/" + selectedIdForDelete,
        });
      });
  }
          ...
          <div className="flex flex-1 flex-col min-w-0">
            ...
            <div style={{ display: "flex", justifyContent: "right",marginTop: "-20px", marginLeft:"10px" }}>
              <div className="add-new-unit">
                {authAccessArr.includes('Add Unit-Retailer') && 
                <Button
                  variant="contained"
                    ...
                    .map((row) => (
                      <TableRow key={row.id}>
                        ...
                        <TableCell
                          ...
                          style={{ maxWidth: 200 }}
                        >
                          {authAccessArr.includes('Edit Unit-Retailer') && 
                          <IconButton
                            style={{ padding: "0" }}
                            onClick={(ev) => {
                              ev.preventDefault();
