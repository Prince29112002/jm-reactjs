...
const CreateStockCode = (props) => {
  ...
  function handleInputChange(event) {
    ...
    if (name === "stockCode") {
      setStockCodeErrTxt("");
      setStockCode(value);
    } else if (name === "purity" && !isNaN(value)) {
      setPurityErrTxt("");
      setPurity(value);
    } else if (name === "weight") {
      setWeight(value);
  ...
  function upadteStockNameDetails() {
    ...
    axios
      .post(Config.getCommonUrl() + "retailerProduct/api/stockname/stockcode", data)
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
          setStockCode("");
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "retailerProduct/api/stockname/stockcode",
          body: data,
        });
      });
  }
    ...
    <div>
      ...
      >
        <div style={modalStyle} className={clsx(classes.paper, "rounded-8")}>
          ...
          <div className="overflow-y-auto" style={{padding: "16px"}}>
            <p className="popup-labl" >Stock name</p>
            <Select
            ...
            <p className="popup-labl" style={{ marginTop: "16px"}}>Stock name description</p>
            <Select
              filterOption={createFilter({ ignoreAccents: false })}
              className="input-select-bdr-dv"
              styles={selectStyles}
            ...
            <p className="popup-labl" style={{ marginTop: "16px"}}>Stock code</p>
            <TextField
              className="input-select-bdr-dv"
            ...
            {StockType === 1 && (
              <div>
                <p className="popup-labl" style={{marginTop: "16px"}}>Purity</p>
                <TextField
                  className="input-select-bdr-dv"
                  ...
                  fullWidth
                />
                <p className="popup-labl" style={{marginTop: "16px"}}>Gold Color</p>
                <Select
                  filterOption={createFilter({ ignoreAccents: false })}
                  className="input-select-bdr-dv"
                  styles={selectStyles}
