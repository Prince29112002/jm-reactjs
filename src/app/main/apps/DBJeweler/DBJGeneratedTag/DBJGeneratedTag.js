...
const DBJGeneratedTag = () => {
  ...
  function getTagedList() {
    Axios.get(
      Config.getCommonUrl() +
        `retailerProduct/api/tagPrintPendingList/tag/printed/list?page=${page}`
    )
      .then((res) => {
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: `retailerProduct/api/tagPrintPendingList/tag/printed/list?page=${page}`,
        });
      });
  }
  ...
  const handleCallDeleteApi = () => {
    ...
    console.log(body);
    var api = `retailerProduct/api/tagPrintPendingList/tag/delete?is_colorstone=${0}`;
...
export default DBJGeneratedTag;
