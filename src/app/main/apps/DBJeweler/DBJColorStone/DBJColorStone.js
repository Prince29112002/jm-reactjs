...
import taggingDataFile from "app/main/SampleFiles/DBJTagging/color_stone_sample_file.csv";
...
const DBJColorStone = () => {
  ...
  const [selectedColumn, setSelectedColumn] = useState(["ITEM_NO"]);
  ...
  function callStockCodeFileUploadApi(formData) {
    ...
    setLoading(true);
    const body = formData;
    var api = "retailerProduct/api/tagPrintPendingList/upload/csv/colorstone";
  ...
  function callExportTaggedDataApi() {
    ...
    var api = "retailerProduct/api/tagPrintPendingList/colorstone/tag/save";
    Axios.post(Config.getCommonUrl() + api, body)
      .then((response) => {
        if (response.data.success) {
        } else {
  ...
  const handleSelectAll = () => {
    if (selectedBarcode.length === apiData.length) {
      ...
      const allRowNames = apiData.map((data) => data.ITEM_NO);
  ...
  const handleSelectedBarcode = (obj) => {
    if (selectedBarcode.includes(obj.ITEM_NO)) {
      const updatedArray = selectedBarcode.filter(
        (item) => item !== obj.ITEM_NO
      );
      const updatedObj = selectedArray.filter(
        (item) => item.ITEM_NO !== obj.ITEM_NO
      );
      ...
      const updateBarcodeData = [...selectedBarcode, obj.ITEM_NO];
            ...
            <Modal open={modalOpen} onClose={handleClose}>
              <Box className={classes.modalStyle}>
                ...
                <div className="p-5 pl-16 pr-16">
                  ...
                  >
                    <a
                      href={taggingDataFile}
                      download="color_stone_sample_file.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
