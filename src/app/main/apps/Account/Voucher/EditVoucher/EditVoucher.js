...
const EditVoucher = (props) => {
  ...
  function getLedgerGroup(id) {
    axios
      .get(Config.getCommonUrl() + `api/ledger/tds/${id}`)
      .then((res) => {
        console.log(res);
        if (id === 3) {
      ...
      .catch((error) => {
        handleError(error, dispatch, { api: `api/ledger/tds/${id}` });
      });
  }

  function getGroupTree() {
    axios
      .get(Config.getCommonUrl() + "api/group/group/tree")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
          ...
          data.map((i) => {
            ...
            if (i.parent_id === null) {
              ...
              if (i[i.id].parent_id === null) {
                ...
                const index = nodes.findIndex(
                  (item) => item.value === i[i.id].parent_id
                );
                if (index > -1) {
                  const innerindex = nodes[index].children.findIndex(
                    (item) => item.value === i.parent_id
                  );
      ...
      .catch((error) => {
        handleError(error, dispatch, { api: "api/group/group/tree" });
      });
  }
  ...
  function getOnChange(checked, nodes) {
    ...
    let array = checked
      ...
      : // : selectedSubDebit.filter((value) => !allNode.includes(value));
        selectedSubDebit.filter((array) =>
          allNode.every(
            (filter) =>
              !(filter.value === array.value && filter.label === array.label)
          )
        );
        ...
        label={
          <FormControlLabel
            control={
              <Checkbox
                disabled={view}
                checked={selectedSubDebit.some(
                  (item) => item.value === x.value && item.label === x.label
                )}
                onChange={(event) =>
  ...
  function getOnCreditChange(checked, nodes) {
    ...
    let array = checked
      ...
      : // : selectedSubCredit.filter((value) => !allNode.includes(value));
        selectedSubCredit.filter((array) =>
          allNode.every(
            (filter) =>
              !(filter.value === array.value && filter.label === array.label)
          )
        ...
        label={
          <FormControlLabel
            control={
              <Checkbox
                disabled={view}
                checked={selectedSubCredit.some(
                  (item) => item.value === x.value && item.label === x.label
                )}
                onChange={(event) =>
  ...
  const editHandler = (id, i) => {
    if (id) {
      ...
      const v_id = props.location.state.id;
      axios
        .get(Config.getCommonUrl() + `api/vouchersettingdetail/${v_id}/${id}`)
        .then((response) => {
        ...
        .catch((error) => {
          setLoading(false);
          handleError(error, dispatch, {
            api: `api/vouchersettingdetail/${v_id}/${id}`,
          });
        });
    }
  };
  ...
  function callAddVoucherApi() {
    ...
    const ledArr = [];
    taxLedgerName.length > 0 &&
      taxLedgerName.map((item) => {
    ...
    if (edit) {
      ...
      var api = `api/vouchersettingdetail/update/${voucherNum}/${updateId}`;
    } else {
      var body = {
      ...
      var api = `api/vouchersettingdetail/${voucherNum}`;
    }
    axios
        ...
        <div className="flex flex-col md:flex-row container">
          ...
          >
            ...
            <div className="main-div-alll" style={{ marginTop: "20px" }}>
              <div className="editvoucher_blg_dv">
                {/* <Card> */}
                {/* <CardContent> */}
                <Grid container>
                  {voucherSettingDetails.length > 0 && (
                    <Grid item xs={12}>
                      <Button
                        className={classes.button}
                        variant="contained"
                  ...
                  <Grid item xs={12}>
                    <Grid container spacing={2}>
                      ...
                      <Grid
                        className="input-details-dv label-d-flex-main"
                        item
                        xs={12}
                        sm={12}
                        md={6}
                          ...
                          <TableBody>
                            {voucherSettingDetails.map((row, i) => (
                              <TableRow key={row.id}>
                                ...
                                <TableCell className={classes.tableRowPad}>
                                  {row.required_other_voucher_no === 1
                                    ? "YES"
                                    : "NO"}
                                </TableCell>
                                <TableCell className={classes.tableRowPad}>
                ...
                <div className="editvoucher_blg_dv">
                        ...
                        >
                          ...
                          >
                            ...
                            {format.value === 1 && (
                              ...
                              >
                                <label>
                                  If vertical formats then what on top
                                </label>
                                <Select
                                  classes={classes}
                                  styles={selectStyles}
                          ...
                          <Grid container className="checkbox-inpu-dv pl-1">
                  ...
                  <Grid item xs={12} className=" checkbox-main-blog">
                    <Grid container>
                      <Grid item xs={12}>
                        <FormControl className="items-center">
                          <FormControlLabel
