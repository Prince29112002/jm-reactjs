...
const VoucherEntry = (props) => {
  ...
  function calculateAfterRate() {
    ...
    if (hsnNum === 1 && isFormatVertical === 1) {
      totalFinalAmt = HelperFunc.getTotalOfField(rows, "Total");
    } else {
      totalFinalAmt = HelperFunc.getTotalOfField(rows, "Amount");
    }
    if (taxTcsTds === 1) {
      //tcs
      tempLedgerAmount = calculatedLedAmount;

      tempfinalAmount = parseFloat(
        parseFloat(totalFinalAmt) + parseFloat(tempLedgerAmount)
      ).toFixed(3);
    } else if (taxTcsTds === 2) {
      // tds
      tempLedgerAmount = parseFloat(
        (parseFloat(totalFinalAmt) * selectedTaxLedRate) / 100
      ).toFixed(3);
      tempfinalAmount = parseFloat(
        parseFloat(totalFinalAmt) - parseFloat(tempLedgerAmount)
      ).toFixed(3);
    } else {
      tempfinalAmount = parseFloat(totalFinalAmt).toFixed(3);
    }
    setCalculatedLedAmount(parseFloat(tempLedgerAmount).toFixed(3));
    setFinalAmoutTotal(parseFloat(tempfinalAmount).toFixed(3));
  }

  function getAllVoucherList() {
    axios
      .get(Config.getCommonUrl() + "api/vouchersettingdetail/other")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
      ...
      .catch((error) => {
        handleError(error, dispatch, { api: "api/vouchersettingdetail/other" });
      });
  }
  ...
  const getLedgerRate = () => {
    axios
      .get(
        Config.getCommonUrl() +
          `api/ledger/tds-tcs/ledger/rate/${ledgerSelectedTax.value}`
      )
      ...
      .catch(function (error) {
        handleError(error, dispatch, {
          api: `api/ledger/tds-tcs/ledger/rate/${ledgerSelectedTax.value}`,
        });
      });
  };
  ...
  const selectedVoucherApiCall = (value) => {
    ...
    axios
      .get(Config.getCommonUrl() + `api/vouchersettingdetail/${v_id}`)
      .then((res) => {
        console.log(res);
        if (res.data.success) {
          ...
          if (arrdata.VoucherSettingDetails.length > 0) {
            ...
            getLedgerList(
              `api/vouchersettingdetail/toDebit/${data.id}`,
              "toDebit"
            );
            getLedgerList(
              `api/vouchersettingdetail/toCredit/${data.id}`,
              "toCredit"
            );
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/vouchersettingdetail/${v_id}`,
        });
      });
    setOpen(false);
  };
  ...
  const handleInputDataChange = (event) => {
    ...
    if (name === "narration") {
      ...
      if (!isNaN(value) && value !== "") {
        ...
        setLedgerAmtErr("");
        let tempfinalAmount = parseFloat(totalFinalAmt) + parseFloat(value);
  ...
  function callAddVoucherEntryApi() {
    ...
    formData.append(
      "taxation_ledger_id",
      ledgerSelectedTax.value ? ledgerSelectedTax.value : null
    );
    ...
    axios
      .post(Config.getCommonUrl() + "api/voucherentry/entry", formData)
      .then((response) => {
        console.log(response);
      ...
      >
        ...
        >
          ...
          <div style={{padding: "30px"}}>
            <div>
              <p style={{paddingLeft: "15px", marginBottom: "10px", color: "#242424"}}>Voucher</p>
              <Select
                classes={classes}
                styles={selectStyles}
                autoFocus
  ...
  const deleteHandler = (delId) => {
    ...
    if (rowData[delId + 1]) {
      ...
      if (isFormatVertical === 1 && !rowData[delId + 1]) {
          ...
          >
            ...
            >
              ...
              >
                ...
                <div className="btn-back" style={{ marginLeft: "10px" }}>
                  {" "}
            ...
            {open && ModalView()}
            <div className="main-div-alll" style={{ marginTop: "20px", display: "flex", flexDirection: "column", minHeight: "calc(100vh - 175px)" }}>
              {!open && (
                <div>
                  <Grid container spacing={2}>
                    <Grid item xs={12} sm={6} md={2} lg={2}>
                      <p className="popup-labl" style={{ marginBottom: "5px" }}>
                        Date
                      </p>
                      <TextField
                        // label="Date"
              ...
              <div className="table_full_width voucherentry-main-table  voucherentry-tabel-fix">
                ...
                >
                  ...
                  {rows.map((element, index) => (
                    <div
                      key={index}
                      className="castum-row-dv voucher-row-dv"
                      // className="all-purchase-tabs mt-5 castum-row-dv"
                      style={{ marginTop: "30px", border: "1px solid #D1D8F5", alignItems: "center" }}
                    >
                      <div
                        className={clsx(classes.tableheader, "delete_icons_dv")}
                      >
              ...
              {taxInclude && (
                <>
                  ...
                  >
                    ...
                    <div className="table-row-source" style={{columnGap: "10px"}}>
                      <TextField
                        value={voucherName}
                        variant="outlined"
                        fullWidth
                      ...
                      />
                      <span style={{ color: "red" }}>
                        {ledgerSelectedTaxErr.length > 0
                          ? ledgerSelectedTaxErr
                          : ""}
                      </span>
                  ...
                  <div
                    className="p-head meta-final-amoount meta-final-amoount-dv"
                    style={{
                      border: "1px solid #D1D8F5",
                      background: "#EBEEFB",
                      fontWeight: "700",
              ...
              >
                {narrationReq === 1 && (
                  ...
                  >
                    <p className="popup-labl" style={{marginBottom: "5px"}}>Narration</p>
                    <TextField
                      // className="mt-16"
