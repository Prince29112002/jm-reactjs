...
const ArticianIssueChainRetailer = (props) => {
  ...
  useEffect(() => {
    let arr = roleOfUser
      ? roleOfUser["Sales-Retailer"]["Artician Issue Metal-Chain-Retailer"]
        ? roleOfUser["Sales-Retailer"]["Artician Issue Metal-Chain-Retailer"]
        : []
      : [];
    const arrData = [];
  ...
  function setFilters() {
    let url =
      "retailerProduct/api/chainJobWorkArticianIssue?department_id=" +
  ...
  function viewHandler(id) {
    props.history.push(
      "/dashboard/sales/articianissuechainretailer/AddArticianIssueChainRetailer",
      {
        id: id,
      }
    );
  ...
  function callDeleteJobWorkMetalRecApi() {
    if (validateDelete()) {
      ...
      axios
        .post(
          Config.getCommonUrl() +
            "retailerProduct/api/chainJobWorkArticianIssue/" +
            selectedIdForDelete,
          body
        )
        .then(function (response) {
        ...
        .catch((error) => {
          handleError(error, dispatch, {
            api:
              "retailerProduct/api/chainJobWorkArticianIssue/" + selectedIdForDelete,
            body,
          });
        });
      handleClose();
          ...
          <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1 pt-20 ">
            ...
            >
              ...
              {authAccessArr.includes(
                "Add /Edit Artician Issue Metal-Retailer"
              ) && (
                <Grid
                  item
                  xs={12}
                ...
                >
                  <Link
                    to="/dashboard/sales/articianissuechainretailer/AddArticianIssueChainRetailer"
            ...
            {loading && <Loader />}
            <div className="main-div-alll pt-16">
              <Grid
                className="department-main-dv "
                container
                spacing={4}
                  ...
                  <Table className={classes.table}>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell className={classes.tableRowPad} width="13%">
                          Customer Name
                        </TableCell>
                        {/* <TableCell className={classes.tableRowPad}>
                        Fine
                      ...
                      {apiData
                        ...
                        .map((row) => (
                          <TableRow key={row.id}>
                            ...
                            {/* <TableCell className={classes.tableRowPad}>
                              {row.ChainJobWorkArticiaIssueOrder[0].gross_weight}
                            </TableCell>
                            <TableCell className={classes.tableRowPad}>
                              {row.ChainJobWorkArticiaIssueOrder[0].net_weight}
                            </TableCell>
                            <TableCell className={classes.tableRowPad}>
                              {parseFloat(row.ChainJobWorkArticiaIssueOrder[0].gold_fine).toFixed(3)}
                            </TableCell>
                            <TableCell className={classes.tableRowPad}>
                              {row.ChainJobWorkArticiaIssueOrder[0].wastage}
                            </TableCell>
                            <TableCell className={classes.tableRowPad}>
                              {parseFloat(row.ChainJobWorkArticiaIssueOrder[0].wastage_fine).toFixed(3)}
                            </TableCell> */}
                            {/* <TableCell className={classes.tableRowPad}>
                              {parseFloat(row.ChainJobWorkArticiaIssueOrder[0].finegold).toFixed(3)}
                            </TableCell> */}
                            ...
                            <TableCell className={classes.tableRowPad} style={{paddingRight: 40}}>
                              {authAccessArr.includes(
                                "View Artician Issue Metal-Retailer"
                              ) && (
                                <IconButton
                                  style={{ padding: "0" }}
                                  onClick={(ev) => {
                              ...
                              {authAccessArr.includes(
                                "Delete Artician Issue Metal-Retailer"
                              ) &&
                                <IconButton
                                  style={{ padding: "0" }}
                                  onClick={(ev) => {
                    ...
                    <TableFooter>
                      <TableRow style={{backgroundColor: "#ebeefb"}}>
                        ...
                        <TableCell className={classes.tableRowPad}>
                        <b style={{display: "block",width: "100%", textAlign: "right"}}>
                            {parseFloat(HelperFunc.getTotalOfFieldNoDecimal(
                              apiData,
                              "totalFineGold"
                            )).toFixed(3)}
                          </b>
            ...
            >
              ...
              <DialogActions style={{justifyContent: "center", columnGap: "15px"}}>
                <Button
                  onClick={handleClose}
...
export default ArticianIssueChainRetailer;
