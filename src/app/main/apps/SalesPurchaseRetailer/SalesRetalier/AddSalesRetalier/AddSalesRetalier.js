...
const AddSalesRetalier = (props) => {
            ...
            if (response.data.data !== null) {
              ...
              temp.map((e) => {
                arrData.push({
                  ...
                  wastage_per: e?.wastage_per,
                  wastage_fine :e?.wastage_fine,
                });
              });
              ...
              setItemName(finalData?.JR_item_name);
  ...
  const handleChange = (index, event) => {
    ...
    } else if (name === "wastage_per" &&  (percentage.test(value) === false || isNaN(value))){
      oldData[index].errors[name] = "Enter valid wastage(%)";
    }
    else if (
  ...
  const handleCatChange = (index, value) => {
    ...
    if (value.is_gold_silver === 1) {
      oldData[index]["rate"] = silverRate;
    } else if (value.is_gold_silver === 0) {
      oldData[index]["rate"] = goldRate;
    }
  ...
  const customCalculation = (oldData, index) => {
    //.......new calculation.............
    const rateConsider = oldData[index].rate_consider;
    const labourConsider = oldData[index].labor_consider;
    const labourPer = oldData[index].labor_per;
    const wasteper = oldData[index].wastage_per ? oldData[index].wastage_per : 0
    const fineWgt = (oldData[index].net_wgt * oldData[index].purity) / 100
    oldData[index].fine_wgt = parseFloat(fineWgt)
    const wastageFinewgt = fineWgt * wasteper / 100
    oldData[index].wastage_fine = parseFloat(wastageFinewgt).toFixed(3)
    

    let wgtRate = 0;
    let wgtLabour = 0;
    const rates = oldData[index].rate ? parseFloat(oldData[index].rate) : 0;
    const tempRate = oldData[index].rate
      ? parseFloat(parseFloat(oldData[index].rate) / 10)
      : 0;
    const addedLabour = oldData[index].laberpergram
      ? oldData[index].laberpergram
      : 0;

      const finalWastageAmt = parseFloat(wastageFinewgt) * parseFloat(tempRate)

    if (rateConsider.value === 1) {
      wgtRate = oldData[index].gross_wgt;
    } else if (rateConsider.value === 2) {
      wgtRate = oldData[index].net_wgt;
    } else if (rateConsider.value === 3) {
      wgtRate = oldData[index].phy_pcs;
    }

    if (rateConsider.value === 3 && wgtRate && rates) {
      oldData[index].rateCharge = parseFloat(
        parseFloat(wgtRate) * parseFloat(rates)
      );
    } else {
      if (wgtRate && tempRate) {
        oldData[index].rateCharge = parseFloat(
          parseFloat(wgtRate) * parseFloat(tempRate)
        );
      }
    }

    if (labourConsider.value === 1) {
      wgtLabour = oldData[index].gross_wgt;
    } else if (labourConsider.value === 2) {
      wgtLabour = oldData[index].net_wgt;
    } else if (labourConsider.value === 3) {
      wgtLabour = oldData[index].phy_pcs;
    }

    if (labourPer.value === 1) {
      oldData[index].totalLabour = parseFloat(
        parseFloat(wgtLabour) * parseFloat(addedLabour)
      );
    } else if (labourPer.value === 2) {
      const perVal = parseFloat(
        parseFloat(wgtLabour) * parseFloat(addedLabour / 100)
      );
      oldData[index].totalLabour = parseFloat(
        parseFloat(perVal) * parseFloat(tempRate)
      );
    } else if (labourPer.value === 3) {
      oldData[index].totalLabour = parseFloat(
        parseFloat(oldData[index].phy_pcs) * parseFloat(addedLabour)
      );
    }

    const rateVal = oldData[index].rateCharge ? oldData[index].rateCharge : 0;
    const laborVal = oldData[index].totalLabour
      ? oldData[index].totalLabour
      : 0;
    const hallmarkVal = oldData[index].total_hallmark_charges
      ? oldData[index].total_hallmark_charges
      : 0;
    const gstdata = oldData[index].gst ? oldData[index].gst : 0;

    const totalAllCharge = parseFloat(
      parseFloat(rateVal) + parseFloat(laborVal) + parseFloat(hallmarkVal) 
    );
    if (gstdata) {
      oldData[index].gstVal = parseFloat(
        (parseFloat(totalAllCharge) * parseFloat(gstdata)) / 100
      );
    } else {
    ...
    oldData[index].amount = parseFloat(
      parseFloat(oldData[index].gstVal) + parseFloat(totalAllCharge) + parseFloat(finalWastageAmt)
    );
  ...
  const deleteRow = (index, id) => {
    ...
    if (oldData[index + 1]) {
      ...
      oldData[index].wastage_per = "";
      oldData[index].wastage_fine = "";
      oldData[index].fine_wgt = "";
  ...
  function handleInputChange(event) {
    ...
    if (name === "goldRate" && !isNaN(value)) {
      ...
      //   numericValue,
      //   numericFinalAmount,
      //   numericValue > numericFinalAmount
      // );
      // if (0 > numericFinalAmount) {
      //   setudharAmountErr("Enter valid amount");
      // } else {
      //   setudharAmountErr("");
      // }
    ...
    oldData.map((item, i) => {
      ...
      if (isAnyDataInrOW(item)) {
        if (item.barcode === "") {
          ...
          item.rate_consider?.value !== 3 &&
          (item.gross_wgt === "" ||
            item.gross_wgt == 0 ||
            grossNetRegex.test(item.gross_wgt) === false)
        ) {
          item.errors.gross_wgt = "Enter valid gross weight";
        } else if (
          item.rate_consider?.value === 3 &&
          (item.gross_wgt === "" ||
            grossNetRegex.test(item.gross_wgt) === false)
          ...
          (item.net_wgt === "" ||
            item.net_wgt == 0 ||
            grossNetRegex.test(item.net_wgt) === false)
        ) {
          item.errors.net_wgt = "Enter valid net weight";
        } else if (
          item.rate_consider?.value === 3 &&
          (item.net_wgt === "" || grossNetRegex.test(item.net_wgt) === false)
        ) {
          item.errors.net_wgt = "Enter valid net weight";
        } else if (
          item.rate_consider?.value !== 3 &&
          ...
          item.rate_consider?.value === 3 &&
          (item.purity === "" || grossNetRegex.test(item.purity) === false)
        ) {
          item.errors.purity = "Enter valid purity";
        } else if (
          item.rate === "" ||
          ...
          item.total_hallmark_charges === "" ||
          grossNetRegex.test(item.total_hallmark_charges) === false
        ) {
          item.errors.total_hallmark_charges = "Enter valid hallmark";
          ...
          item.errors.laberpergram = "Enter valid labour";
        }
      }
    });
    setPackingSlipData(oldData);
    ...
    axios
      ...
      .then(function (response) {
        ...
        if (response.data.success === true) {
          ...
          const chargeType = makingCharge.type;
          const charge = makingCharge.value;
          const considerRate =
            temp.ProductCategory.is_gold_silver === 0 ? goldRate : silverRate;
          ...
          if (chargeType === "INR") {
            ...
            chargeValue = { value: 1, label: "Per Gram" };
            usedCharge = parseFloat(charge);
            finalCharge = parseFloat(
              parseFloat(temp.gross_wgt) * parseFloat(usedCharge)
            );
          } else if (chargeType === "PER") {
            displayCharge = charge;
            chargeValue = { value: 2, label: "Per (%)" };
            usedCharge = parseFloat(parseFloat(considerRate) / 10);
            const perVal = parseFloat(
              parseFloat(temp.gross_wgt) * parseFloat(displayCharge / 100)
            );
            finalCharge = parseFloat(
              parseFloat(perVal) * parseFloat(usedCharge)
            );
          } else if (chargeType === "PCS") {
            displayCharge = charge;
            chargeValue = { value: 3, label: "Per Pcs" };
            usedCharge = parseFloat(charge);
            finalCharge = parseFloat(
              parseFloat(temp.phy_pcs) * parseFloat(usedCharge)
            );
          }
          const tempRate = parseFloat(
            (parseFloat(temp.net_wgt) * parseFloat(considerRate)) / 10
          );
          const gstVALUE = parseFloat(
            parseFloat(
              (parseFloat(tempRate) +
                parseFloat(finalCharge) +
                parseFloat(temp.total_hallmark_charges)) *
                gst
            ) / 100
          );
          ...
          newTempData.rate = considerRate;
          newTempData.wastage_per = temp?.wastage_per ? temp?.wastage_per : 0;
          newTempData.wastage_fine = temp?.wastage_fine ? temp?.wastage_fine : 0;
          newTempData.total_hallmark_charges = temp.total_hallmark_charges ? temp.total_hallmark_charges : 0;
          newTempData.laberpergram = displayCharge ? displayCharge : 0;
          newTempData.labor_per = chargeValue ? chargeValue : 0;
          newTempData.totalLabour = finalCharge ? finalCharge : 0;
          newTempData.gst = temp.ProductCategory?.hsn_master?.gst;
          newTempData.gstVal = parseFloat(parseFloat(gstVALUE));
          newTempData.amount = parseFloat(
            parseFloat(tempAmount) + parseFloat(temp.total_hallmark_charges)
          );
          newTempData.rateCharge = parseFloat(parseFloat(tempRate));
  ...
  const calCulateTotalcall = (arr, addedRoundoff) => {
    const tempTotalGosswgt = HelperFunc.getTotalOfField(arr, "gross_wgt");
    const tempTotalNetwgt = HelperFunc.getTotalOfField(arr, "net_wgt");
    const tempTotalHallmark = HelperFunc.getTotalOfField(
      arr,
      "total_hallmark_charges"
    );
    ...
    const tempSubTotal = HelperFunc.getTotalOfField(arr, "rateCharge");
    ...
    const tempInvoiceAmt = parseFloat(
      parseFloat(tempSubTotal) +
        parseFloat(tempTotalHallmark) +
        parseFloat(tempTotalLabour) +
        parseFloat(tempTotalGst) +
        parseFloat(roundOffValue)
    );
    // console.log(tempTotalAmt,"innnnnnnnnnnn")
    setTotaleGrossWight(
      isNaN(tempTotalGosswgt) ? 0 : parseFloat(tempTotalGosswgt)
    );
    setTotalNetWeight(isNaN(tempTotalNetwgt) ? 0 : parseFloat(tempTotalNetwgt));
    setTotalHallMasrk(
      isNaN(tempTotalHallmark) ? 0 : parseFloat(tempTotalHallmark)
    );
    ...
    setSubTotal(isNaN(tempSubTotal) ? 0 : parseFloat(tempSubTotal));
  ...
  function createFromPackingSlip(resetFlag, toBePrint) {
    ...
    packingSlipData.map((x) => {
      if (x.barcode) {
        Orders.push({
          ...
          hsn: x.hsn,
          gross_wgt: parseFloat(x.gross_wgt),
          net_wgt: parseFloat(x.net_wgt),
          purity: parseFloat(x.purity),
          rate: parseFloat(x.rate),
          rate_charge: parseFloat(x.rateCharge),
          phy_pcs: parseInt(x.phy_pcs),
          total_hallmark_charges: parseFloat(x.total_hallmark_charges),
          ...
          fine_weight: x.fine_wgt,
          wastage_per: x.wastage_per,
          wastage_fine: x.wastage_fine,
        });
      }
    });
    ...
    const body = {
      ...
      cash: caseamunt === 0 || caseamunt === "" ? null : parseFloat(caseamunt),
      card:
        cardamount === 0 || cardamount === "" ? null : parseFloat(cardamount),
      online:
  ...
  const handleInputFocus = (event) => {
    ...
    if (name === "JradvanceAmount") {
      if (parseFloat(JrAdvanceAmount) === 0) {
        setJrAdvanceAmount("");
      }
    }

    if (name === "udharAmount") {
      if (parseFloat(udharAmount) === 0) {
        setUdharAmount("");
      }
    }
                ...
                <Grid item xs={5} sm={4} md={4} lg={5} key="1">
                  <FuseAnimate delay={300}>
                    <Typography className="text-18 font-700">
                      {isView ? "View Sales Invoice " : "Add Sales Invoice "}
                    </Typography>
                  </FuseAnimate>
                </Grid>
            ...
            <div className="main-div-alll">
              ...
              >
                  ...
                  <Table className={classes.table}>
                    <TableHead>
                      <TableRow>
                        ...
                        <TableCell className={classes.tableRowPad} width={120}>
                          Billing Category
                        </TableCell>
                        <TableCell className={classes.tableRowPad}>
                          HSN
                        ...
                        <TableRow key={index}>
                          ...
                          <TableCell className={classes.tablePad}>
                            <Select
                              ...
                              classes={classes}
                              styles={selectStyles}
                              options={rateConsiderArr.map((suggestion) => ({
                                value: suggestion.value,
                                label: suggestion.label,
                              }))}
                              value={element.rate_consider}
                              onChange={(e) => {
                                handleRateCalChange(index, e);
                              }}
                              isDisabled={isView}
                            />
                          </TableCell>
                          <TableCell className={classes.tablePad}>
                            <Select
                              ...
                              classes={classes}
                              styles={selectStyles}
                              options={labourConsiderArr.map((suggestion) => ({
                                value: suggestion.value,
                                label: suggestion.label,
                                isDisabled:
                                  element.rate_consider.value === 3 &&
                                  suggestion.value !==
                                    element.rate_consider.value,
                              }))}
                              value={element.labor_consider}
                              onChange={(e) => {
                                handleLabourCalChange(index, e);
                              }}
                              isDisabled={isView}
                            />
                          </TableCell>
                          <TableCell className={classes.tablePad}>
                            <TextField
                              ...
                              className={"addconsumble-dv"}
                              error={
                                element.errors && element.errors.gross_wgt
                                  ? true
                                  : false
                              }
                              helperText={
                                element.errors && element.errors.gross_wgt
                                  ? element.errors.gross_wgt
                                  : ""
                              }
                              variant="outlined"
                              fullWidth
                          ...
                          <TableCell className={classes.tablePad}>
                            <TextField
                              name="net_wgt"
                              value={
                                isView ? element.net_wgt : element.net_wgt || ""
                              }
                              onChange={(e) => handleChange(index, e)}
                              className={"addconsumble-dv"}
                              error={
                                element.errors && element.errors.net_wgt
                                  ? true
                                  : false
                              }
                              helperText={
                                element.errors && element.errors.net_wgt
                                  ? element.errors.net_wgt
                                  : ""
                              }
                              variant="outlined"
                              fullWidth
                          ...
                          <TableCell className={classes.tablePad}>
                            <TextField
                              ...
                              error={
                                element.errors &&
                                element.errors.total_hallmark_charges
                                  ? true
                                  : false
                              }
                              helperText={
                    ...
                    <TableFooter>
                      <TableRow className={classes.tableFooter}>
                        ...
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}>
                          <b>{parseFloat(TotaleGrossWight).toFixed(3)}</b>
                        </TableCell>
                        <TableCell className={classes.tableRowPad}>
                          <b>{parseFloat(TotalNetWeight).toFixed(3)}</b>
                        </TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}>
                          <b>{parseFloat(TotalHallMasrk).toFixed(2)}</b>
                        </TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                        <TableCell className={classes.tableRowPad}></TableCell>
                ...
                >
                  ...
                  >
                    <label className="mr-2">Labour : </label>
                    <label className="ml-2">
                      {isView
                ...
                >
                  ...
                  >
                    <label className="mr-2">Hallmark : </label>
                    <label className="ml-2">
                      {" "}
                      {isView
                        ? Config.numWithComma(TotalHallMasrk)
                        : parseFloat(TotalHallMasrk).toFixed(2)}
                    </label>
                  </div>
                </div>
                  ...
                  >
                    ...
                    <label className="ml-2 input-sub-total">
                      <TextField
                        ...
                        disabled={isView || subTotal === "" || subTotal === 0}
                        value={roundOff}
                ...
                <Paper style={{ overflow: "auto" }}>
                  ...
                  >
                    ...
                    <TextField
                        ...
                        className={"ml-2 addconsumble-dv"}
                        value={itemName}
                        error={itemNameErr.length > 0 ? true : false}
...
export default AddSalesRetalier;
