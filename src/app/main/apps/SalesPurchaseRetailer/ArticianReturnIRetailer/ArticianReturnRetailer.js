...
const ArticianReturnRetailer = (props) => {
  ...
  useEffect(() => {
    let arr = roleOfUser
      ? roleOfUser["Sales-Retailer"]["Artician Return Metal-Retailer"]
        ? roleOfUser["Sales-Retailer"]["Artician Return Metal-Retailer"]
        : []
      : [];
    const arrData = [];
  ...
  function setFilters() {
    let url =
      "retailerProduct/api/jobworkarticianreturn?department_id=" +
  ...
  function viewHandler(id) {
    props.history.push(
      "/dashboard/sales/articianreturnretailer/addarticianreturnretailer",
      {
        id: id,
      }
    );
  ...
  function callDeleteArticianReturnApi() {
    if (validateDelete()) {
      ...
      axios
        .post(
          Config.getCommonUrl() +
            "retailerProduct/api/jobworkarticianreturn/" +
            selectedIdForDelete,
          body
        )
        .then(function (response) {
          ...
          <div className="flex flex-1 flex-col min-w-0 makeStyles-root-1">
            ...
            >
              ...
              {authAccessArr.includes(
                ...
                >
                  <Link
                    to="/dashboard/sales/articianreturnretailer/addarticianreturnretailer"
                        ...
                        .map((row) => (
                          <TableRow key={row.id}>
                            ...
                            >
                              ...
                              {Config.numWithComma(row.JobWorkArticianReturnOrder[0].rate_of_fine_gold)}
                            </TableCell>
...
export default ArticianReturnRetailer;
