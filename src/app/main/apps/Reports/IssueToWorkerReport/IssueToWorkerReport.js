...
const IssueToWorkerReport = (props) => {
  ...
  const [issueToWorkerData, setIssueToWorkerData] = useState([]);
  ...
  const exportToExcel = (type, fn, dl) => {
    if (issueToWorkerData.length > 0) {
      ...
      return dl
        ...
        : XLSX.writeFile(wb, fn || `Issue_To_Worker.${type || "xlsx"}`);
    } else {
  ...
  console.log(castingFromDate);
  function postIssueToworkStation() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/productionreport/issue/workstation`,
        body
      )
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
          console.log(response.data.data);
          setIssueToWorkerData(response.data.data);
        } else {
          dispatch(Actions.showMessage({ message: response.data.message }));
        }
      })
      .catch((error) => {
        handleError(error, dispatch, {
          api: `api/productionreport/issue/workstation`,
          body: body,
        });
      });
  }
...
export default IssueToWorkerReport;
