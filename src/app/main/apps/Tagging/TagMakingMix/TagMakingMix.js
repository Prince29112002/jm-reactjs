...
import EditVariant from "../TagMakingLot/EditVariant/EditVariant";
...
const TagMakingMix = (props) => {
  ...
  function callLotDetailsApi(lotInput) {
    ...
    axios
      .get(
        Config.getCommonUrl() +
          `api/lot/Number/${lotInput}/${selectedDepartment.value.split("-")[1]}`
      )
      .then((res) => {
        console.log(res);
        if (res.data.success) {
        ...
        if (res.data.success) {
          ...
          if (apiData.DesignData.length > 0 && stockData.length > 0) {
            ...
            apiData.DesignData.map((item) => {
              ...
              moreTotalWgt += parseFloat(item.weight);
              moreTotalPcs += parseFloat(item.pcs);
              moreData.push({
                stockNo: apiData.variant_number,
  ...
  function callCreateLotApi() {
    ...
    displayData.map((item) => {
      body[`${item.description} Pcs`] = item.pcs / variantArrData.Balance_Pcs;
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.success) {
          ...
          dispatch(
            Actions.showMessage({
              message: "Tagging Mix Added Successfully",
              variant: "success",
            })
          );
          setLoading(false);
  ...
  function weightValidation() {
    var Regex = /[1-9][0-9]*(?:\/[1-9][0-9])*/g;
    if (!edtGrossWeight || Regex.test(edtGrossWeight) === false) {
      setEdtGrossWtErr("Please Enter Valid Weight");
  ...
  function callEditWeightApi() {
    axios
      .put(
        Config.getCommonUrl() +
          "api/lotdetail/regenerate/" +
          selectedDataForEdit.id,
        {
          // itemName: edtGrossWeight,
          lot_id: selectedDataForEdit.id,
          gross_wgt: 11,
          net_wgt: 100,
          details_json: {
            Variant: "FNBL71001X25 ",
            purity: 91.8,
            "Total Weight": "1.935",
            "Total Amt": "0.262",
            "Stone Weight": "0.971",
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "api/lotdetail/regenerate/" + selectedDataForEdit.id,
          body: {
            // itemName: edtGrossWeight,
            lot_id: selectedDataForEdit.id,
            gross_wgt: 11,
  ...
  function handleSubmitAllVariant() {
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/lotdetail/submit/lot/details/${lotId}`)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          dispatch(
    ...
    <div className={clsx(classes.root, props.className, "w-full")}>
      <FuseAnimate animation="transition.slideUpIn" delay={200}>
        <div className="flex flex-col md:flex-row container">
          <div className="flex flex-1 flex-col min-w-0 lot-tabel-main">
            {loading && <Loader />}
            {open === true && (
              <Modal
            ...
            {open === false && (
              <>
                <Grid
                  className="tagmarking-btn-fullwidth"
                  container
                  spacing={4}
                  alignItems="stretch"
                  style={{ margin: 0 }}
                  ...
                  <Grid container spacing={3}>
                    ...
                    >
                      <Select
                        ...
                        placeholder="Party Rate Profile"
                      />
                      <span style={{ color: "red" }}>
                ...
                <Grid container spacing={3}>
                  <Grid className="tabel-left-width-dv" style={{ padding: 10 }}>
                    <Grid item xs={12} style={{ padding: 0 }}>
                      ...
                      {showmoreArr.length > 0 && (
                        ...
                        >
                          <a
                            className="btn btn-primary tagmaking-show-btn   w-224 mx-auto mt-0"
                            onClick={showMore}
                          >
                  ...
                  <Grid className="img-box-blg-main">
                    <Grid className="img-box-blg-dv" style={{ padding: 20 }}>
                      <img src={image} />
                    </Grid>
                  </Grid>
                  ...
                  {AllVariants.length > 0 ? (
                    <>
                      <Grid item xs={12}>
                        <div className="mr-16 ml-16 mt-32 tagmaking_mix_tbl view_making-first-tbl-mix">
