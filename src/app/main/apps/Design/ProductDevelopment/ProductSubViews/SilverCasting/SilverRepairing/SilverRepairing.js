...
import { SilverRepairingPrintComp } from "./PrintComp/SilverRepairingPrintComp";
...
const SilverRepairing = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 5, tab: 3 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Silver_Repair_" + getDateAndTime(),
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/reparing/${JobNo}?tab=5`)
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: `api/cadjob/reparing/${JobNo}?tab=5`,
        });
      });
  }
  ...
  function uploadCadCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/silvercasting/cad/repair/read`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: `api/silvercasting/cad/repair/read`,
  ...
  function callCADRepairingapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/silvercasting/repair/issue`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/silvercasting/repair/issue`,
          body: body,
        });
      });
  }
              ...
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Silver_Casting_repairing_issue.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
...
export default SilverRepairing;
