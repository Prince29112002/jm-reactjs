...
import { ReceiveFromSilverPrintCom } from "./ReceiveFromSilverPrintCom/ReceiveFromSilverPrintCom";
...
const ReceiveFromSilver = (props) => {
  const [printObj, setPrintObj] = useState({ remark: "", designerName: "", partynumber: "" })
  const componentRef = React.useRef(null);
  const onBeforeGetContentResolve = React.useRef(null);
  const handleAfterPrint = () => { //React.useCallback
    //resetting after print 
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 5, tab: 2, })
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Silver_Receive_From_orker" + getDateAndTime(),
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "remark") {
      ...
      setInum(value);
      setInumErr("");
      setPrintObj({ ...printObj, partynumber: value })
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/silvercasting/csv/read`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: `api/silvercasting/csv/read`,
  ...
  function callCAmReceiveWorkerapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/silvercasting/receive`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/silvercasting/receive`, body: body });
      });
  }
                ...
                >
                  ...
                  <div className="model-row-blg-dv">
                    ...
                    <Grid
                      ...
                      style={{ padding: 6 }}
                    >
                      <a href={sampleFile} download="Silver_Casting_receive_from_worker.csv" >Download Sample </a>
                    ...
                    <div style={{ display: "none" }}>
                      <ReceiveFromSilverPrintCom ref={componentRef} printObj={printObj} />
                    </div>
                  </div>
                </Grid>
              </Grid>
...
export default ReceiveFromSilver;
