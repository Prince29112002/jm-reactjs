...
const SilverCasting = ({ props }) => {
  ...
  useEffect(() => {
    const arr = roleOfUser['Product Development']?.['Silver Casting']
    const arrData = []
    if (arr.length > 0) {
      arr.map((item) => {
  ...
  useEffect(() => {
    if (modalView === 6) {
      ...
      History.push(`/dashboard/design/issuesilvercasting`);
    } else if (modalView === 3) {
      History.push(`/dashboard/design/receivefromsilver`);
    } else if (modalView === 4) {
      History.push(`/dashboard/design/silverrepairing`);
    } else if (modalView === 5) {
      History.push(`/dashboard/design/silverrejection`);
    }
  }, [modalView]);
  ...
  function getAllDataList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/silvercasting")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
      ...
      .catch((error) => {
        setExportData([]);
        setLoading(false);
        handleError(error, dispatch, { api: "api/silvercasting" });
      });
  }
  ...
  function callSetAllData(temp) {
    const arrData = temp.map((item) => {
      return {
        cadJobNum: item.CadNumberSilver !== null ? item.CadNumberSilver.CadJobDetails?.CadJobNumber?.cad_number : "",
        voucher_no: item.voucher_no !== null ? item.voucher_no : "",
        cad_number:
          item.CadNumberSilver !== null
            ? item.CadNumberSilver.temp_cad_no
            : "",
        designerName: item.Worker !== null ? item.Worker.username : "",
        weight:
          item.CadNumberSilver.cam_weight !== null
            ? item.CadNumberSilver.cam_weight
            : "",
        size: item.CadNumberSilver.size !== null ? item.CadNumberSilver.size : "",
        process:
          item.process !== null
            ? item.process === 1
              ? "Issue to worker"
    ...
    let tempDlData = temp.map((item) => {
      return {
        "CAD Number": item.CadNumberSilver !== null
          ? item.CadNumberSilver.temp_cad_no
          : "",
        "Voucher Number": item.voucher_no !== null ? item.voucher_no : "",
        "CAD Job Number": item.CadNumberSilver !== null ? item.CadNumberSilver.CadJobDetails?.CadJobNumber?.cad_number : "",
        ...
        "Weight":
          item.CadNumberSilver.cam_weight !== null
            ...
            : "",
        "Size":
          item.CadNumberSilver.size !== null
            ? item.CadNumberSilver.size
            : "",
        "Process":
          item.process !== null
            ? item.process === 1
  ...
  function completedList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/silvercasting/completed/all")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
          const temp = res.data.data;
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: "api/silvercasting/completed/all",
        });
      });
  }
  ...
  function callCompletedData(temp) {
    const arrData = temp.map((item) => {
      return {
        jobNum:
          item.CadNumberSilver?.CadJobDetails?.CadJobNumber?.cad_number,
        cadJobNum:
          item.CadNumberSilver !== null ? item.CadNumberSilver.temp_cad_no : "",
        cadWeight:
          item.CadNumberSilver.cam_weight !== null
            ? item.CadNumberSilver.cam_weight
            : "",
        ...
        endDate:
          item !== null
            ...
            : "",
        // status: "pendding",
        id: item.CadNumberSilver !== null ? item.CadNumberSilver.id : null,
  ...
  const ButtonArr = [
    ...
    { id: 4, text: "Silver Casting Repairing Issue" },
    { id: 5, text: "Silver Casting Repairing Receive" },
    { id: 6, text: "Silver Casting Completed" },
  ];
  ...
  function transferProcess() {
    if (processValidation()) {
      axios
        .put(Config.getCommonUrl() + `api/cadjob/process/transfer/${cadJobId}/4/${selectedTransferOpts.value}/${mainId}`)
        .then(function (response) {
          console.log(response);
        ...
        .catch((error) => {
          ...
          handleError(error, dispatch, {
            api: `api/cadjob/process/transfer/${cadJobId}/4/${selectedTransferOpts.value}/${mainId}`
          })
  ...
  function callSearchFilterApi() {
    setLoading(true);
    if (modalView === 6) {
      var api = `api/silvercasting/completed/search?endDate=${endDate}&search=${searchData}`
    } else {
      var api = `api/silvercasting/search/?endDate=${endDate}&search=${searchData}`
    }
    axios
                  ...
                  >
                    ...
                    >
                      <CSVLink
                        ...
                        data={exportData}
                        filename={modalView === 1 ? "silverCasting.csv" : "Completed_silverCasting_list.csv"}
                      >
                        Download
                      </CSVLink>
                    </Button>
                          ...
                          {cadList
                            .map((row, index) => (
                              <TableRow key={index}>
                                ...
                                <TableCell className={classes.tableRowPad}>
                                  {parseFloat(row.cadWeight).toFixed(3)}
                                </TableCell>
                                <TableCell className={classes.tableRowPad}>
                                  {row.size}
                                </TableCell>
                ...
                <div className="p-5 pl-16 pr-16">
                  ...
                  <Select
                    ...
                    options={transferOpts
                      .filter(x => x.label !== "Silver Casting").map((suggestion) => ({
