...
import { SilverRejectionPrintComp } from "./PrintComp/SilverRejectionPrintComp";
...
const SilverRejection = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Silver_Repairing_Received_" + getDateAndTime(),
  ...
  function callRepairingReceiveApi() {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/silvercasting/repair/receive`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          ...
          History.push(`/dashboard/design`, { view: 1, sub: 5 });
        } else {
          dispatch(
            Actions.showMessage({
              message: response.data.message,
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/silvercasting/repair/receive`,
          body: body,
        });
      });
  }
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/silvercasting/csv/read/issue/received`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/silvercasting/csv/read/issue/received`,
            ...
            >
              ...
              >
                {/* <Button
                  ...
                  size="small"
                  onClick={() =>
                    History.push(`/dashboard/design`, { view: 1, sub: 5, tab: 4 })
                  }
                >
                  Back
                </Button> */}
              ...
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Silver_Casting_repairing_receive.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
...
export default SilverRejection;
