...
import { CadrepairingPrintComp } from "./PrintComp/CadrepairingPrintComp";
...
const CADRepairing = (props) => {
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/reparing/${JobNo}?tab=1`)
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: `api/cadjob/reparing/${JobNo}?tab=1`,
        });
      });
  }
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "endDate") {
      ...
      if(e.target.files[0].type==="text/csv"){
        ...
        document.getElementById("refCADNumber").value = "";
      }
    }
  };
  ...
  function uploadCadCsv() {
    const formData = new FormData();
    formData.append("file", refCADNumberCSV);
    setLoading(true);
    axios
      .post(
        Config.getCommonUrl() + `api/cadjobreceivedesign/csv/repair`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        ...
        document.getElementById("refCADNumber").value = "";
        handleError(error, dispatch, {
          api: `api/cadjobreceivedesign/csv/repair`,
  ...
  function callCADRepairingapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/cadjobreceivedesign/repair/cad`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/cadjobreceivedesign/repair/cad`,
          body: body,
        });
      });
  }
            ...
            >
              ...
              >
                {/* <Button
                  ...
                  size="small"
                  onClick={() =>
                    History.push(`/dashboard/design`, { view: 1, sub: 2, tab: 2 })
                  }
                >
                  Back
                </Button> */}
            ...
            <div className="main-div-alll">
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  ...
                  <Grid item lg={4} md={6} sm={6} xs={6} style={{ padding: 6 }}>
                  ...
                  >
                    <a href={sampleFile} download="CAD_repairing_issue.csv">
                      Download Sample{" "}
                    </a>
                  </Grid>
                  ...
                  <Grid
                    ...
                    sm={12}
                    xs={12}
                    className="mb-16 table_scroll mt-16"
                  >
                    <>
