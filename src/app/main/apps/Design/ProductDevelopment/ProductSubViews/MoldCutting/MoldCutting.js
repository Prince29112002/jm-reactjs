...
const SilverCasting = ({ props }) => {
  ...
  const [completedMoldList, setCompletedMoldList] = useState([]);
  ...
  useEffect(()=>{
    const arr = roleOfUser['Product Development']?.['Mold Cutting'] 
    const arrData = []
    if(arr.length > 0) {
      arr.map((item)=>{
  ...
  useEffect(() => {
    if (modalView === 6) {
      ...
      History.push(`/dashboard/design/issuetomold`);
    } else if (modalView === 3) {
      History.push(`/dashboard/design/receivefrommold`);
    } else if (modalView === 4) {
      History.push(`/dashboard/design/moldrepairing`);
    } else if (modalView === 5) {
      History.push(`/dashboard/design/moldrejection`);
    }
  }, [modalView]);
  ...
  //     History.push(`/dashboard/design/IsueeToMold`);
  //   } else if (addModal && addModal === 2) {
  //     History.push(`/dashboard/design/ReceiveFromMold`);
  //   } else if (addModal && addModal === 3) {
  //     History.push(`/dashboard/design/MoldRepairing`);
  //   } else if (addModal && addModal === 4) {
  //     History.push(`/dashboard/design/MoldRejection`);
  //   }else {
  ...
  //       setModalView(1)
  //     } else if (props.location.state.tab === 2) {
  //       setModalView(2)
  //     }
  //     else if (props.location.state.tab === 3) {
  //       setModalView(3)
  //     } else if (props.location.state.tab === 4) {
  //       setModalView(4)
  //     }
  //   } else {
  //     setModalView(1)
  ...
  function completedList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/moldcutting/completed/all")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: "api/moldcutting/completed/all",
        });
      });
  }
  ...
  function callCompletedData(temp){
          const arrData = temp.map((item) => {
            return {
              cadJobNum:
                item.FinalCadNumberMold?.temp_cad_no,
              jobNum:
                item.FinalCadNumberMold?.CadJobDetails?.CadJobNumber?.cad_number,
              designerName:
                item.Worker !== null ? item.Worker.username : "",
              weight:
                item.cam_weight,
              ...
              id: item.FinalCadNumberMold !== null ? item.FinalCadNumberMold.id : null,
          ...
          let tempDlData = temp.map((item) => {
            return {
              ...
              " Designer Name":
                item.Worker !== null ? item.Worker.username : "",
              "Weight":
                item.cam_weight !== null ? item.cam_weight:"" ,
              "Size":
  ...
  function getAllDataList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/moldcutting")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
          const temp = res.data.data;
      ...
      .catch((error) => {
        setExportData([])
        setLoading(false);
        handleError(error, dispatch, { api: "api/moldcutting" });
      });
  }
  ...
  function transferProcess() {
    if (processValidation()) {
      axios
        .put(Config.getCommonUrl() + `api/cadjob/process/transfer/${cadJobId}/6/${selectedTransferOpts.value}/${mainId}`)
        .then(function (response) {
          console.log(response);
        ...
        .catch((error) => {
          ...
          handleError(error, dispatch, {
            api: `api/cadjob/process/transfer/${cadJobId}/6/${selectedTransferOpts.value}/${mainId}`
          })
  ...
  function callSetAllData(temp) {
    const arrData = temp.map((item) => {
      return {
        ...
        cad_number: item.FinalCadNumberMold !== null ? item.FinalCadNumberMold.temp_cad_no : '',
        cadJobNum:
          item.FinalCadNumberMold !== null
            ? item.FinalCadNumberMold.CadJobDetails?.CadJobNumber?.cad_number
            : "",
        designerName: item.Worker !== null ? item.Worker.username : "",
        weight:
          item.FinalCadNumberMold.cam_weight !== null
            ? item.FinalCadNumberMold.cam_weight
            : "",
        size: item.FinalCadNumberMold.size !== null ? item.FinalCadNumberMold.size : "",
        process:
          item.process !== null
            ? item.process === 1
              ? "Issue to worker"
    ...
    let tempDlData = temp.map((item) => {
      return {
        "CAD Number": item.FinalCadNumberMold !== null ? item.FinalCadNumberMold.temp_cad_no : '',
        ...
        "Weight":
          item.FinalCadNumberMold.cam_weight !== null
            ...
            : "",
        "Size": 
          item.FinalCadNumberMold.size !== null
           ? item.FinalCadNumberMold.size
            : "",
        "Process":
          item.process !== null
            ? item.process === 1
  ...
  const  ButtonArr = [
      ...
      { id: 4, text: "Mold Cutting Repairing Issue" },
      { id: 5, text: "Mold Cutting Repairing Receive" },
      { id: 6, text: "Mold Cutting Completed" },
    ]
  ...
  function callSearchFilterApi() {
    setLoading(true);
    if(modalView === 6){
      var api = `api/moldcutting/completed/search?endDate=${endDate}&search=${searchData}`
    }else{
      var api = `api/moldcutting/search/?endDate=${endDate}&search=${searchData}`
    }
    axios
                ...
                >
                  ...
                  >
                    <CSVLink
                      ...
                      data={exportData}
                      filename={modalView === 1 ? "mold_cutting.csv" : "Completed_mold_cutting_list.csv"}
                    >
                     Download
                    </CSVLink>
                  </Button>
                          ...
                          {moldList
                            .map((row, index) => (
                              ...
                              >
                                ...
                                <TableCell className={classes.tableRowPad} style={{paddingRight:"30px"}}>
                                  {/* {row.status} */}
                                  {row.endDate !== ""
                                    ? displayStatus(row)
                                    : ""}{" "}
