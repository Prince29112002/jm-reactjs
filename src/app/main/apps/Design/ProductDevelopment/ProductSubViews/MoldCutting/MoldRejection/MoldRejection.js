...
import { MoldRejectionPRintComp } from "./PrintComp/moldRejectionPrintComp";
...
const MoldRejection = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 7, tab: 4 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Mold_Rejection" + getDateAndTime(),
  ...
  function callRepairingReceiveApi() {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/moldcutting/repair/receive`, body)
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/moldcutting/repair/receive`,
          body: body,
        });
      });
  }
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/moldcutting/csv/read/issue/received`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/moldcutting/csv/read/issue/received`,
              ...
              <div className="pb-8 mt-8 w-full">
                <Grid container>
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Mold_cutting_repairing_receive.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
