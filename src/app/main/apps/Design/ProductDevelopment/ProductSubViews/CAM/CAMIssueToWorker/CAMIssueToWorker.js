...
import { CAMIssueToWorPrintCom } from "./CAMIssueToWorPrintCom/CAMIssueToWorPrintCom";
...
const CAMIssueToWorker = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 3, tab: 1 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "CAM_Issue_To_Worker" + getDateAndTime(),
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/cad/${JobNo}?tab=3`)
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/cadjob/cad/${JobNo}?tab=3` });
      });
  }
  ...
  function getVoucherNum() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/camJob/getVoucher`)
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
          setVoucherNum(response.data.data.voucherNo);
          setPrintObj({ ...printObj, voucherNo: response.data.data.voucherNo });
        } else {
          dispatch(
            Actions.showMessage({
              message: response.data.message,
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/camJob/getVoucher` });
      });
  }
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "endDate") {
      ...
      setCadErr("");
        ...
        dispatch(Actions.showMessage({ message: "Accept only csv format",variant: "error",}));
  ...
  function uploadMasterIssueCsv() {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/camJob/cad/issued/read`, formData)
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        ...
        document.getElementById("fileinput").value = "";
        handleError(error, dispatch, {
          api: `api/camJob/cad/issued/read`,
  ...
  function callIssueToworker(isPrint) {
    const body = {
      ...
      end_date: endDate,
      remark: remark,
      cadJobIds: selectDesign.length > 0 ? selectDesign : null,
    };
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/camJob/issuedToJobWorker`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/camJob/issuedToJobWorker`,
          body: body,
        });
      });
  }

  const uploadCadFile = (e,id,index) => {
    const arrData = [...cadNumList]
    arrData[index].fileName = e.target.files[0].name
    setCadNumList(arrData)
    const formData = new FormData();
    formData.append("file",e.target.files[0])
    axios.put(Config.getCommonUrl() + `api/camjob/stl/upload/${id}`,formData)
    .then((response) => {
      if(response.data.success){
    ...
    .catch((error) => {
      handleError(error, dispatch, { api: `api/camjob/stl/upload/${id}`, body: JSON.stringify(formData) })
    })
}
              ...
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  ...
                  <Grid
                    ...
                    className="mt-10"
                  >
                    <a href={sampleFile} download="CAM_issue_to_worker.csv">
                      Download Sample{" "}
                    </a>
                  </Grid>
                  <Grid
                      ...
                      <MaUTable className={classes.table}>
                        ...
                        <TableBody>
                          {cadNumList.map((item, index) => (
                            <TableRow key={index}>
                              <TableCell style={{ textAlign: "start" }} className={classes.tableRowTd}>
                                <FormControlLabel
                                  key={item.id}
                              ...
                              <TableCell className={(classes.tableRowTd)}>
                            <Button
                                size="small"
                              ...
                              >
                                ...
                                <TextField
                                type="file"
                                onChange={(e)=>uploadCadFile(e,item.id,index)}
                                style={{ display: "none" }}
                              /> </label>
                              </Button>
...
export default CAMIssueToWorker;
