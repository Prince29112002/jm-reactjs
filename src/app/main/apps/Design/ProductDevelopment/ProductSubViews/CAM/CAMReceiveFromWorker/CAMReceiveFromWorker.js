...
import { CAMReceiveFromWorkerPrintComp } from "./PrintComp/CAMReceiveFromWorkerPrintComp";
...
const CAMReceiveFromWorker = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 3, tab: 2 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "CAM_Receive_From_Worker_" + getDateAndTime(),
  ...
  const handleInputChange = (e) => {
    ...
    if (name === "remark") {
      ...
      setInum(value);
      setInumErr("");
      setPrintObj({ ...printObj, inum: value });
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/camJob/cad/receive/read`, formData)
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        ...
        document.getElementById("fileinput").value = "";
        handleError(error, dispatch, {
          api: `api/camJob/cad/receive/read`,
  ...
  function callCAmReceiveWorkerapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/camJob/receive`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
            ...
            History.push(`/dashboard/design`, { view: 1, sub: 3, tab: 5 });
          }
          // props.modalSave();
          dispatch(
            Actions.showMessage({
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/camJob/receive`, body: body });
      });
  }
                  ...
                  >
                    ...
                    <div className="model-row-blg-dv">
                      ...
                      >
                        <a
                          href={sampleFile}
                          download="CAM_receive_from_worker.csv"
                        >
                          Download Sample{" "}
                        </a>
                      </Grid>
...
export default CAMReceiveFromWorker;
