...
import { MasterRejectionReceivedPrintComp } from "./PrintComp/MasterRejectionReceivedPrintComp";
...
const MasterRejectionReceived = (props) => {
  const [printObj, setPrintObj] = useState({ remark: "", cadjobNumber: "", workerName: "", processType: "" })
  ...
  const handleAfterPrint = () => { //React.useCallback
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 4, tab: 4 })
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Master_Repairing_Received_" + getDateAndTime(),
  ...
  function checkforPrint() {
    handlePrint()
    // if(validateFile() && validateWorker() && validateNextProcess()){
  ...
  function callRepairingReceiveApi() {
    ...
    setLoading(true)
    axios
      .post(Config.getCommonUrl() + `api/cammasterfinish/repair/receive`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          dispatch(Actions.showMessage({ message: response.data.message,variant:"success" }));
          History.push(`/dashboard/design`, { view: 1, sub: 4, })
        }  else {
          dispatch(Actions.showMessage({ message: response.data.message,variant: "error" }));
        }
        setLoading(false);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/cammasterfinish/repair/receive`, body: body });
      });
  }
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() +
        `api/cammasterfinish/csv/read/issue/received`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: `api/cammasterfinish/csv/read/issue/received`,
          ...
          <div className="flex flex-1 flex-col min-w-0  makeStyles-root-1 pt-20">
            ...
            <div className="main-div-alll">
              ...
              <Grid container spacing={3}>
                ...
                <Grid item lg={3} md={4} sm={6} xs={12} style={{ padding: 6 }}>
                ...
                <Grid item lg={3} md={4} sm={6} xs={12} style={{ padding: 6 }}>
                  <label>Next process</label>
                    ...
                    classes={classes}
                    // styles={selectStyles}
                    options={processTypeData.filter(x=>x.label != "Master Finish")}
                    value={processType}
                    onChange={handleChangePocessType}
                    placeholder="Select next process"
                  />
                  <span style={{ color: "red" }}>
                  ...
                  <Grid item lg={3} md={4} sm={6} xs={12} style={{ padding: 6, display: 'flex', alignItems: 'center' }}>
                    <a href={sampleFile} download="Master_Finish_repairing_receive.csv" >Download Sample </a>
                ...
                <div style={{ display: "none" }}>
                  <MasterRejectionReceivedPrintComp ref={componentRef} printObj={printObj} />
                </div>
              </Grid>
              </div>
            </div>
...
export default MasterRejectionReceived;
