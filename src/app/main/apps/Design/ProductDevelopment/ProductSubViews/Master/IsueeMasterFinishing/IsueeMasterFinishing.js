...
import { IsueeMasterFinishingPrintComp } from "./PrintComp/IsueeMasterFinishingPrintComp";
...
const IsueeMasterFinishing = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 4, tab: 1 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Assign_For_Master_Finish_" + getDateAndTime(),
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/cad/${JobNo}?tab=4`)
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/cadjob/cad/${JobNo}?tab=4` });
      });
  }
  ...
  function getVoucherNum() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cammasterfinish/getVoucher`)
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/cammasterfinish/getVoucher` });
      });
  }
  ...
  function uploadMasterIssueCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/cammasterfinish/cad/issue/read`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/cammasterfinish/cad/issue/read`,
  ...
  function callIssueToworker(isPrint) {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/cammasterfinish/issuedToJobWorker`,
        body
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/cammasterfinish/issuedToJobWorker`,
          body: body,
        });
      });
  }
              ...
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Master_Finish_issue_to_worker.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
...
export default IsueeMasterFinishing;
