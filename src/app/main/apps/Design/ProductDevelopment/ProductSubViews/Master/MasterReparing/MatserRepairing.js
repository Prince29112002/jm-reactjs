...
import { MasterReparingPrintCom } from "./MasterReparingPrintCom/MasterReparingPrintCom";
...
const MatserRepairing = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 4, tab: 3 });
  };
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/reparing/${JobNo}?tab=4`)
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: `api/cadjob/reparing/${JobNo}?tab=4`,
        });
      });
  }
  ...
  function uploadCadCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/cammasterfinish/cad/repair/read`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/cammasterfinish/cad/repair/read`,
  ...
  function callCADRepairingapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/cammasterfinish/repair/issue`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/cammasterfinish/repair/issue`,
          body: body,
        });
      });
  }
              ...
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Master_Finish_repairing_issue.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
...
export default MatserRepairing;
