...
const ChromePlating = ({ props }) => {
  ...
  useEffect(() => {
    const arr = roleOfUser["Product Development"]?.["Chrome Plating"];
    const arrData = [];
    if (arr.length > 0) {
      arr.map((item) => {
  ...
  useEffect(() => {
    if (modalView === 6) {
      ...
      History.push(`/dashboard/design/issuetochrome`);
    } else if (modalView === 3) {
      History.push(`/dashboard/design/receivefromchrome`);
    } else if (modalView === 4) {
      History.push(`/dashboard/design/chromerepairing`);
    } else if (modalView === 5) {
      History.push(`/dashboard/design/chromerejection`);
    }
  }, [modalView]);
  ...
  //     History.push(`/dashboard/design/IsueeToCrome`);
  //   } else if (addModal && addModal === 2) {
  //     History.push(`/dashboard/design/ReceiveFromCrome`);
  //   } else if (addModal && addModal === 3) {
  //     History.push(`/dashboard/design/CromeRepairing`);
  //   } else if (addModal && addModal === 4) {
  //     History.push(`/dashboard/design/CromeRejection`);
  //   } else {
  //     History.push(`/dashboard/design`, { view: 1, sub: 6, tab: 1, });
  //   }
  // }, [addModal]);
  ...
  function completedList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/chromeplating/completed/all")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: "api/chromeplating/completed/all",
        });
      });
  }
  ...
  function callCompletedData(temp) {
    const arrData = temp.map((item) => {
      return {
        jobNum:
          item.FinalCadNumberChrome?.CadJobDetails?.CadJobNumber?.cad_number,
        cadJobNum: item.FinalCadNumberChrome?.temp_cad_no,
        ...
        status: "pending",
        id:
          item.FinalCadNumberChrome !== null
            ? item.FinalCadNumberChrome.id
            : null,
    ...
    let tempDlData = temp.map((item) => {
      return {
        "CAD Job Number":
          item.FinalCadNumberChrome?.CadJobDetails?.CadJobNumber?.cad_number,
        "CAD Number": item.FinalCadNumberChrome?.temp_cad_no,
  ...
  function callSetAllData(temp) {
    const arrData = temp.map((item) => {
      return {
        ...
        cad_number:
          item.FinalCadNumberChrome !== null
            ? item.FinalCadNumberChrome.temp_cad_no
            : "",
        cadJobNum:
          item.FinalCadNumberChrome?.CadJobDetails?.CadJobNumber?.cad_number,
        designerName: item.Worker !== null ? item.Worker.username : "",
        weight:
          item.FinalCadNumberChrome.cam_weight !== null
            ? item.FinalCadNumberChrome.cam_weight
            : "",
        size:
          item.FinalCadNumberChrome.size !== null
            ? item.FinalCadNumberChrome.size
            : "",
        process:
          item.process !== null
            ? item.process === 1
  ...
  function getAllDataList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/chromeplating")
      .then((res) => {
        console.log(res);
        if (res.data.success) {
          const temp = res.data.data;
      ...
      .catch((error) => {
        setExportData([]);
        setLoading(false);
        handleError(error, dispatch, { api: "api/chromeplating" });
      });
  }
  ...
  function transferProcess() {
    if (processValidation()) {
      axios
        .put(
          Config.getCommonUrl() +
          `api/cadjob/process/transfer/${cadJobId}/5/${selectedTransferOpts.value}/${mainId}`
        )
        .then(function (response) {
          console.log(response);
        ...
        .catch((error) => {
          handleError(error, dispatch, {
            api: `api/cadjob/process/transfer/${cadJobId}/5/${selectedTransferOpts.value}/${mainId}`,
          });
        });
    }
  }
  ...
  const ButtonArr = [
    ...
    { id: 4, text: "Chrome Plating Repairing Issue" },
    { id: 5, text: "Chrome Plating Repairing Receive" },
    { id: 6, text: "Chrome Plating Completed" },
  ];
  ...
  function callSearchFilterApi() {
    setLoading(true);
    if (modalView === 6) {
      var api = `api/chromeplating/completed/search?endDate=${endDate}&search=${searchData}`;
    } else {
      var api = `api/chromeplating/search/?endDate=${endDate}&search=${searchData}`;
    }
    axios
            ...
            <div className="main-div-alll">
                    ...
                    >
                      <CSVLink
                        ...
                        filename={
                          modalView === 1
                            ? "chrome_platting.csv"
                            : "Completed_chrome_platting_list.csv"
                        }
                      >
                        Download
                      </CSVLink>
              ...
              <div className="design_list_tbl">
                <Paper
                  className={clsx(
                    classes.tabroot,
                    "mt-32 table-responsive design_list_blg view_design_list_blg"
                  )}
                >
                  {modalView === 6 ? (
                    <>
                ...
                <div className="p-5 pl-16 pr-16">
                  ...
                  <Select
                    ...
                    options={transferOpts
                      .filter((x) => x.label !== "Chrome Plating")
                      .map((suggestion) => ({
