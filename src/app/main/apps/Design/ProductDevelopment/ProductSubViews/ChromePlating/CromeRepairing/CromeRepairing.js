...
import { CromeRepairingPrintComp } from "./PrintComp/CromeRepairingPrintComp";
...
const CromeRepairing = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 6, tab: 3 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Crome_Repair_" + getDateAndTime(),
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/reparing/${JobNo}?tab=6`)
      ...
      .catch((error) => {
        ...
        handleError(error, dispatch, {
          api: `api/cadjob/reparing/${JobNo}?tab=6`,
        });
      });
  }
  ...
  function uploadCamRepairCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/chromeplating/cad/repair/read`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/cad/repair/read`,
  ...
  function callCADRepairingapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/chromeplating/repair/issue`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          if (isPrint) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/repair/issue`,
          body: body,
        });
      });
  }
            ...
            <div className="main-div-alll">
              <div className="model-row-blg-dv">
                ...
                >
                  <a
                    href={sampleFile}
                    download="Chrome_Plating_repairing_issue.csv"
                  >
                    Download Sample{" "}
                  </a>
                </Grid>
