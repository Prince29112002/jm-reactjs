...
import { CromeRejectionPrintCom } from "./CromeRejectionPrintCom/CromeRejectionPrintCom";
...
const CromeRejection = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 6, tab: 4 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Chrome Repairing Received" + getDateAndTime(),
  ...
  function callRepairingReceiveApi() {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/chromeplating/repair/receive`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          ...
          History.push(`/dashboard/design`, { view: 1, sub: 6 });
        } else {
          dispatch(
            Actions.showMessage({
              message: response.data.message,
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/repair/receive`,
          body: body,
        });
      });
  }
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/chromeplating/csv/read/issue/received`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/csv/read/issue/received`,
              ...
              <div className="pb-8  pl-8 pr-8 mt-8">
                <Grid container spacing={3}>
                  <Grid
                    ...
                    style={{ padding: 6 }}
                  >
                    <p className="popup-labl p-4 ">Worker Name</p>
                    <Select
                      classes={classes}
                      // styles={selectStyles}
                  ...
                  <Grid
                    ...
                    style={{ padding: 6 }}
                  >
                    <p className="popup-labl p-4 ">Next Process</p>
                    <Select
                      filterOption={createFilter({ ignoreAccents: false })}
                      classes={classes}
                      // styles={selectStyles}
                      options={processTypeData.filter(
                        (x) => x.label != "Chrome Plating"
                      )}
                      value={processType}
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Chrome_Plating_repairing_receive.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
