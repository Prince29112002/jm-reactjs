...
import { ReceiveFromCromePrintComp } from "./PrintComp/ReceiveFromCromePrintComp";
...
const ReceiveFromCrome = (props) => {
  ...
  const handleAfterPrint = () => {
    ...
    // checkAndReset()
    History.push(`/dashboard/design`, { view: 1, sub: 6, tab: 2 });
  };
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "chrome_Receive_From_Worker_" + getDateAndTime(),
  ...
  function uploadReceiveJobWorkerCsv() {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/chromeplating/csv/read`, formData)
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/csv/read`,
  ...
  function callCAmReceiveWorkerapi(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/chromeplating/receive`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          dispatch(
                  ...
                  >
                    ...
                    <div className="model-row-blg-dv">
                      ...
                      >
                        <a
                          href={sampleFile}
                          download="Chrome_Plating_receive_from_worker.csv"
                        >
                          Download Sample{" "}
                        </a>
                      </Grid>
...
export default ReceiveFromCrome;
