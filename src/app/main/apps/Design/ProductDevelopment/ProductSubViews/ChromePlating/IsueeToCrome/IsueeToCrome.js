...
import { IsueeToCromePrintCom } from "./IsueeToCromePrintCom/IsueeToCromePrintCom";
...
const IsueeToCrome = (props) => {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "Chrome_Issue_To_Worker" + getDateAndTime(),
  ...
  function getCADData(JobNo) {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/cadjob/cad/${JobNo}?tab=6`)
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/cadjob/cad/${JobNo}?tab=6` });
      });
  }
  ...
  function getVoucherNum() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + `api/chromeplating/getVoucher`)
      .then(function (response) {
        console.log(response);
        if (response.data.success === true) {
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, { api: `api/chromeplating/getVoucher` });
      });
  }
  ...
  function uploadMasterIssueCsv() {
    ...
    axios
      .post(
        Config.getCommonUrl() + `api/chromeplating/cad/issue/read`,
        formData
      )
      .then((response) => {
        console.log(response);
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/cad/issue/read`,
  ...
  function callIssueToworker(isPrint) {
    ...
    setLoading(true);
    axios
      .post(Config.getCommonUrl() + `api/chromeplating/issuedToJobWorker`, body)
      .then((response) => {
        console.log(response);
        if (response.data.success) {
          dispatch(
      ...
      .catch((error) => {
        setLoading(false);
        handleError(error, dispatch, {
          api: `api/chromeplating/issuedToJobWorker`,
          body: body,
        });
      });
  }
              ...
              <div className="w-full mr-0 pr-0">
                ...
                <Grid container>
                  ...
                  >
                    <a
                      href={sampleFile}
                      download="Chrome_Plating_issue_to_worker.csv"
                    >
                      Download Sample{" "}
                    </a>
                  </Grid>
