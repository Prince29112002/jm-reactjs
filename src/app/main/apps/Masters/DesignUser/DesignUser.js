...
import CreateDesignUser from "./subview/CreateDesignUser";
...
const DesignUser = (props) => {
  ...
  function getDesignUserList() {
    setLoading(true);
    axios
      .get(Config.getCommonUrl() + "api/admin?is_design_master=1")
      ...
      .catch(function (error) {
        setLoading(false);
        handleError(error, dispatch, { api: "api/admin?is_design_master=1" });
      });
  }
              ...
              >
                {ButtonArr.map((btndata, idx) => {
                  if (defaultView != btndata.id) {
                    return btndata.text == "Create Designer" ? (
                      <Button
