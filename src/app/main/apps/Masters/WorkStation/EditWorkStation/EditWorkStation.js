...
const EditWorkStation = (props) => {
  ...
  useEffect(() => {
    ...
    if (dataToBeEdited !== undefined) {
      ...
      setWorkStationName(dataToBeEdited.row.name);
      setSelectedDepartment({
        value: dataToBeEdited.row.department.id,
        label: dataToBeEdited.row.department.name,
      });

      // dataToBeEdited.row.workstation_worker.name

      const workerData = dataToBeEdited.row.workstation_worker.map((x) => {
        return {
          id: x.id,
          name: x.name,
          errors: {
      ...
      setWorkStationFormValues(workerData);

      //
      const processData = dataToBeEdited.row.workstation_process.map((x) => {
        return {
          id: x.id,
          value: x.process ? x.process.id : null,
          label: x.process ? x.process.process_name : null,
          errors: {
            label: null,
          },
        };
      });
      setProcessDataFormValues(processData);
    }
  ...
  function callEditWorkStationApi() {
    ...
    let workerData = workStationformValues.map((x) => {
      if (x.id !== "") {
        return {
          id: x.id,
          name: x.name,
    ...
    axios
      .put(Config.getCommonUrl() + "api/workstation/" + dataToBeEdited.row.id, {
        // code: vendorCode,
      ...
      .catch((error) => {
        handleError(error, dispatch, {
          api: "api/workstation/" + dataToBeEdited.row.id, body: {
            // code: vendorCode,
            name: workStationName,
            department_id: selectedDepartment.value,
            workerData: workerData,
              ...
              <div>
                ...
                >
                  ...
                  <div className="w-full">
                        ...
                        <>
                          <div style={{ display: "flex"}} key={index}>
                            ...
                            <TextField
                              ...
                              onChange={(e) => handleWorkStationChange(index, e)}
                              variant="outlined"
                              fullWidth
                            />
                           </div>
                            ...
                            {index === workStationformValues.length - 1 && (
                              ...
                              id="btn-save"
                              variant="contained"
                              className="w-130 mt-28"
                              style={{ height: "37px", marginLeft: "15px" }}
                              aria-label="Register"
                                onClick={() => {
                                  addWorkStationFormFields();
                                }}
                              >
                                Add Worker
                              </Button>
                    ...
                    <div
                      className="mt-16"
                      style={{ border: "1px solid #EBEEFB", width: "340px", borderRadius: "7px 7px 0px 0px", borderBottom: "none", borderLeft: "none", borderRight: "none" }}
                    >
                      <div style={{ background: "#EBEEFB", borderRadius: "7px 7px 0px 0px", height: '37px' }}>
                        <Typography
                          style={{ fontWeight: "700", padding: "10px", paddingLeft: "20px" }}
                        >
                          Worker Name
                        </Typography>
                      </div>
                    ...
                    <div className="add-client-row" style={{ paddingTop: "20px" }}></div>

                    {/* <div style={{ display: "flex", paddingTop: "35px" }}> */}
                      {processDataformValues.map((element, index) => (
                        <>
                          <div style={{ display: "flex", paddingTop: "15px"}} key={index}>
                          <div style={{ width: "340px", paddingTop: "10px" }}>
                ...
                <Button
                 id="btn-save"
                  variant="contained"
                  className="w-150 mx-auto mt-32 float-right"
                  aria-label="Register"
                  onClick={(e) => {
                    handleFormSubmit(e);
                  }}
